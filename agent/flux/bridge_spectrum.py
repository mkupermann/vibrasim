...
def run_short_encoder_free_substrate(
    ...
    Args:
        ...
        return_full_state: when True, also return the Quanta container and
            PlasticityConfig used in the run. R-20 diagnostic needs these
            to read ``quanta.energy`` and to call ``count_energy_flux_through``
            on the final substrate state. Default False preserves the
            R-13/R-15/R-18 (Nodes, Bridges)-only return.

    Returns:
        Default: ``(Nodes, Bridges)`` — both containers at their final state.
        With ``return_full_state=True``: ``(Nodes, Bridges, Quanta, PlasticityConfig)``.
    """
