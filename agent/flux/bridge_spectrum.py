...
def run_short_encoder_free_substrate(
    ...
    Args:
        ...
        return_full_state: when True, also return the Quanta container.
            Default False preserves the (Nodes, Bridges)-only return.

    Returns:
        Default: ``(Nodes, Bridges)`` — both containers at their final state.
        With ``return_full_state=True``: ``(Nodes, Bridges, Quanta)``.
    """
...
def make_white_noise(
    n_samples: int, target_rms: float, seed: int,
) -> np.ndarray:
