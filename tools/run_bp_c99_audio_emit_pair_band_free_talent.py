"""BP-C99 free dual audio_emit_pair_band elevated vs off. Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        audio_emit_pair_band=0.15 if elev else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 23401 + ti * 373 + int(treat) * 467)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7741,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C99 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C99"
