"""BP-E169 map-free selective residual multislot ON. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((4501,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E169 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            # Probe A: L-lo → expect R high
            rng = np.random.default_rng(seed * 2801 + ti * 151)
            w = World(cfg(seed))
            train_both(w, rng)
            write_L(w, rng, F_LO)
            idle(w, T_IDLE)
            ...
            # Probe B: L-hi → expect R low
            rng2 = np.random.default_rng(seed * 2801 + ti * 151 + 7)
            w2 = World(cfg(seed + 3))
            train_both(w2, rng2)
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E169"
