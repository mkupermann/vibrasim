"""BP-E118 hard multi-trial R0 silence, dual soft restore, hard silence again. Headless."""
...
def r0_silent_both(w):
    w.k_charge[:]=0; w.k_latch[:]=0
    fire_phase(w,[L0])
    ok0 = end_near(w,R0)<=0.25 and end_near(w,R1)>=1.0
    w.k_charge[:]=0; w.k_latch[:]=0
    fire_phase(w,[L1])
    ok1 = end_near(w,R0)<=0.25 and end_near(w,R1)>=1.0
    return ok0 and ok1

def both_fanout(w):
    w.k_charge[:]=0; w.k_latch[:]=0
    fire_phase(w,[L0])
    ok0 = end_near(w,R0)>=1.0 and end_near(w,R1)>=1.0
    w.k_charge[:]=0; w.k_latch[:]=0
    fire_phase(w,[L1])
    ok1 = end_near(w,R0)>=1.0 and end_near(w,R1)>=1.0
    return ok0 and ok1

def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument("--smoke",action="store_true")
    args=p.parse_args(argv)
    seeds,trials=((3281,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E118 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*141217+ti*811)
            w=World(cfg(seed)); train_all(w,rng)
            dual_cut_all(w)
            for k in ARMS:
                restore(w,rng,k)
            hard_cut(w,"00"); hard_cut(w,"10")
            b1s.append(r0_silent_both(w))
            restore(w,rng,"00"); restore(w,rng,"10")
            b2s.append(both_fanout(w))
            hard_cut(w,"00"); hard_cut(w,"10")
            b3s.append(r0_silent_both(w))
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.80
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E118","bars":{
        "B1_first_hard_silence":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_dual_restore":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_second_hard_silence":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E118"; out.mkdir(parents=True,exist_ok=True)
