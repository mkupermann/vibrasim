"""BP-C24 free dual talent with ILW port-seed bootstrap vs free-only control. Headless."""
...
N_SIDE, N_SEED, T_FULL = 400, 40, 1200
SEEDS, TRIALS = (3481, 3491, 3501), 3
LOW, HIGH = (100.0, 2000.0), (500.0, 10000.0)
MID = 40.0
PL = np.array([20., 25., 25.]); PR = np.array([60., 25., 25.])
...
def port_seed(w, rng):
    for _ in range(N_SEED):
        apply_ilw_port_event(w, PL, rng, seed_freq=400.)
        apply_ilw_port_event(w, PR, rng, seed_freq=5000.)
...
def run_one(seed, ti, with_seed, t_total):
    w=World(cfg(seed))
    rng=np.random.default_rng(seed*1601+ti*109+int(with_seed)*19)
    if with_seed:
        port_seed(w, rng)
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((3481,),1,300) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C24 start smoke={args.smoke}")
    on,off=[],[]
    for s in seeds:
    ...
    result={"id":"BP-C24","bars":{
        "B1_seed_spec":{"value":b1,"threshold":0.90,"pass":p1},
        "B2_ctrl_spec":{"value":b2,"threshold":0.80,"pass":p2},
        "B3_seed_pop":{"value":b3,"threshold":0.80,"pass":p3},
        "B4_delta":{"value":b4,"threshold":0.15,"pass":p4},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-C24"; out.mkdir(parents=True,exist_ok=True)
