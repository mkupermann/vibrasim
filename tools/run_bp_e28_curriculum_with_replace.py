"""BP-E28 curriculum overwrite with PRIM8 pair replace. Headless."""
...
def make_cfg(seed):
    return WorldConfig(
        ...
        ilw_pair_replace_enabled=True, neuron_dynamics_enabled=True, theta_fire=2., t_refractory=0.02, n_emit=0,
        bridge_charge_prop_rate=2., bridge_prop_min_strength=0., charge_latch_enabled=True, charge_latch_tau=0.,
    )
...
def latch_partner(w,Li):
    ...
    for t in range(T_PROP):
        if t%10==0 and w.k_alive[Li]: w.k_charge[Li]=thr+5.
        tick(w,dt)
    idle(w,T_END)
    best_i,best_v=-1,-1.
...
def match_rate(w,pairs):
    ...
    ok=n=0
    for Li in Ls:
        fL=float(w.k_freq[Li]); pred=latch_partner(w,Li)
        if pred<=0: continue
        n+=1; true=true_partner(fL,pairs)
...
def main(argv=None):
    ...
    seeds,trials=((911,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E28 start smoke={args.smoke}")
    b1s,b2s,b3s,b4s=[],[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*47003+ti*197)
            w=World(make_cfg(seed)); train_map(w,rng,MAP_A); train_map(w,rng,MAP_B)
            rB,has=match_rate(w,MAP_B); rA,_=match_rate(w,MAP_A)
            b1s.append(rB); b2s.append(rA); b4s.append(has and len(bridged_L(w))>=1)
            w2=World(make_cfg(seed)); train_map(w2,rng,MAP_A)
            rAo,_=match_rate(w2,MAP_A); b3s.append(rAo)
    ...
    out=Path.home()/".eqmod"/"bet"/"BP-E28"; out.mkdir(parents=True, exist_ok=True)
