"""BP-C98 free dual workspace_broadcast_strength low vs default (self_aware ON). Headless."""
...
def cfg(seed, treat):
    return WorldConfig(
        ...
        workspace_broadcast_strength=0.3 if treat else 1.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 23001 + ti * 367 + int(treat) * 461)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7701,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C98 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C98"
