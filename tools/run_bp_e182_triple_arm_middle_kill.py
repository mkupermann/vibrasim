"""BP-E182 triple-arm middle R1 hard kill; c0 and c2 survive. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((4861,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E182 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 4601 + ti * 71)
            w = World(cfg(seed))
            train_all(w, rng)
            hard_cut_at(w, ARMS[1][1])  # R1 middle
            idle(w, 10)
            clear_state(w)
    ...
    result = {"id": "BP-E182", "bars": {
        "B1_c0_survives": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_c1_fails": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_c2_survives": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E182"
