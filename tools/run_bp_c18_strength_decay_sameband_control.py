"""BP-C18 strength-decay same-band negative control vs dual-band positive. Headless."""
...
def run_one(seed, ti, same_band, t_total):
    w=World(cfg(seed))
    rng=np.random.default_rng(seed*2609+ti*83+int(same_band)*31)
    if same_band:
        inject(w,rng,N_SIDE,8,32,SAME[0],SAME[1])
        inject(w,rng,N_SIDE,48,72,SAME[0],SAME[1])
    else:
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((2331,),1,300) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C18 start smoke={args.smoke}")
    same,dual=[],[]
    for s in seeds:
        for ti in range(trials):
            same.append(run_one(s,ti,True,t_tot))
            dual.append(run_one(s,ti,False,t_tot))
    b1=float(np.mean([1 if r[1] else 0 for r in same]))
    b2=float(np.mean([1 if r[0] else 0 for r in same]))
    b3=float(np.mean([1 if r[1] else 0 for r in dual]))
    p1,p2,p3=b1<=0.60,b2>=0.80,b3>=0.80
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-C18","bars":{
        "B1_sameband_spec":{"value":b1,"threshold":0.60,"pass":p1},
        "B2_sameband_pop":{"value":b2,"threshold":0.80,"pass":p2},
        "B3_dualband_spec":{"value":b3,"threshold":0.80,"pass":p3},
    },"verdict":verdict,"tau":TAU}
    out=Path.home()/".eqmod"/"bet"/"BP-C18"; out.mkdir(parents=True,exist_ok=True)
