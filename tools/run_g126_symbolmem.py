...
CELLS = [6.0, 12.0, 18.0, 24.0]   # K=4, pitch 6 (> G97 ~3)
CELLR = 1.5
K = 4
...
if __name__ == "__main__":
    print(f"=== G126: store+recall hex nibbles in matter memory (cells pitch 6) ===", flush=True)
    ...
    print("\n--- VERDICT ---", flush=True)
    print(f"G126 all nibbles recalled exactly (both seeds): {allok}", flush=True)
    print(("G126: PASS - the matter memory STORES and RECALLS written hex-nibble symbols (persistent written memory, no LLM)"
           if allok else "G126: NULL/PARTIAL - some nibbles mis-recalled (see output)"), flush=True)
