"""BP-E78 hybrid path-switch curriculum AND <-> OR. Headless."""
...
def restore_L1M(w,rng):
    link(w,rng,L1,M,400.,1500.,N_RES)

def restore_L3R(w,rng):
...
def or_only(w):
    return peak_R(w,[L3])>=1.0 and peak_R(w,[L1,L2])<=0.25

def and_only(w):
    return peak_R(w,[L1,L2])>=1.0 and peak_R(w,[L3])<=0.25

def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument("--smoke",action="store_true")
    args=p.parse_args(argv)
    seeds,trials=((2251,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E78 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*101097+ti*523)
            w=World(cfg(seed)); train(w,rng)
            # OR-only: cut AND, ensure OR up
            soft_cut_at(w,I1)
            restore_L3R(w,rng)
            b1s.append(or_only(w))
            # AND-only: cut OR, restore AND
            soft_cut_at(w,I3)
            restore_L1M(w,rng)
            b2s.append(and_only(w))
            # OR-only again
            soft_cut_at(w,I1)
            restore_L3R(w,rng)
            b3s.append(or_only(w))
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.75
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E78","bars":{
        "B1_or_only":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_and_only":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_or_only_again":{"value":a3,"threshold":0.75,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E78"; out.mkdir(parents=True,exist_ok=True)
