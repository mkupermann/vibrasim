"""BP-C26 free dual talent with charge_latch_tau>0 vs tau=0 control. Headless."""
...
def cfg(seed, latch_tau):
    return WorldConfig(
        ...
        neuron_dynamics_enabled=True, charge_latch_enabled=True, charge_latch_tau=float(latch_tau),
    )
...
def run_one(seed, ti, latch_on, t_total):
    w=World(cfg(seed, TAU if latch_on else 0.0))
    rng=np.random.default_rng(seed*1801+ti*113+int(latch_on)*29)
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((4081,),1,300) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C26 start smoke={args.smoke}")
    on,off=[],[]
    for s in seeds:
    ...
    result={"id":"BP-C26","bars":{
        "B1_latch_spec":{"value":b1,"threshold":0.90,"pass":p1},
        "B2_ctrl_spec":{"value":b2,"threshold":0.80,"pass":p2},
        "B3_latch_pop":{"value":b3,"threshold":0.80,"pass":p3},
        "B4_delta":{"value":b4,"threshold":0.15,"pass":p4},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-C26"; out.mkdir(parents=True,exist_ok=True)
