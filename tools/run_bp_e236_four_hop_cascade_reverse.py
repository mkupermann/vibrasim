"""BP-E236 four-hop cascade reverse dual fire-select. Headless."""
...
SEEDS, TRIALS = (7121, 7131), 6
N_TRAIN, N_WRITE, T_PROP = 10, 10, 140
NEAR = 7.0
# path0 y=15; path1 y=35 — five ports each (4 hops)
P0 = [np.array([10. + i * 14., 15., 25.]) for i in range(5)]
P1 = [np.array([10. + i * 14., 35., 25.]) for i in range(5)]
F0 = (300.0, 700.0, 1500.0, 3500.0, 8000.0)
F1 = (8000.0, 3500.0, 1500.0, 700.0, 300.0)
...
def main(argv=None):
    ...
    seeds, trials = ((7121,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E236 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 17201 + ti * 173)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_near(w, P0[4])
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E236"
