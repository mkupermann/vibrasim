...
def run(seed):
    ...
    # (c) hybrid: GF(2) on the local learner's LOW-CONFIDENCE residual (lower half by |feel|)
    conf = np.array([abs(L0.feel(x)) for x in Xtr])
    lowc = conf <= np.median(conf)
    s_res = gf2_solve((Xtr[lowc] < 0).astype(np.int64), (ytr[lowc] < 0).astype(np.int64))
...
if __name__ == "__main__":
    print("=== HYB-02: hybrid decomposes a MIXED rule (gate + SQ-hard parity) ===", flush=True)
    seeds = [0, 7]
    R = {}
    for s in seeds:
        R[s] = run(s)
    ...
    HYB02a = all(0.65 <= R[s]['acc_raw'] <= 0.85 for s in seeds)
    HYB02b = all(R[s]['acc_gf2'] <= 0.80 for s in seeds)
    HYB02c = all(R[s]['acc_hyb'] >= 0.93 for s in seeds)
    passed = HYB02a and HYB02b and HYB02c

    print("\n--- VERDICT ---", flush=True)
    print(f"HYB02a raw local partial (0.65-0.85)   : {HYB02a}", flush=True)
    print(f"HYB02b pure GF(2) fails (<=0.80)       : {HYB02b}", flush=True)
    print(f"HYB02c hybrid decomposes both (>=0.93) : {HYB02c}", flush=True)
    ...
    print(f"\nHYB-02: {verdict}", flush=True)
    out = Path.home() / ".eqmod" / "bet" / "HYB02"; out.mkdir(parents=True, exist_ok=True)
    (out / "result.json").write_text(json.dumps({"R": {str(s): R[s] for s in seeds}, "passed": passed,
                                                  "HYB02a": HYB02a, "HYB02b": HYB02b, "HYB02c": HYB02c}, indent=2, default=str))
