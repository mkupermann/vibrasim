"""BP-E208 reverse fire-select long-idle under G12. Headless."""
...
SEEDS, TRIALS = (5881, 5891), 6
N_TRAIN, N_WRITE, T_PROP, T_IDLE = 12, 12, 80, 400
MID = 40.0
...
def main(argv=None):
    ...
    seeds, trials, t_idle = ((5881,), 2, 50) if args.smoke else (SEEDS, TRIALS, T_IDLE)
    print(f"BP-E208 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 9501 + ti * 57)
            w = World(cfg(seed))
    ...
    result = {"id": "BP-E208", "bars": {
        "B1_rev_c0_idle": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_rev_c1_idle": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_both": {"value": a3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E208"
