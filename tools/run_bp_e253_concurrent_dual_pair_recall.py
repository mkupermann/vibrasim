"""BP-E253 concurrent dual pair-link selective recall. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((7601,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E253 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 22001 + ti * 271)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_near_both(w, L0, L1)
            r0 = peak_near(w, R0) >= 1.0
            r1 = peak_near(w, R1) >= 1.0
            b1s.append(r0)
            b2s.append(r1)
    ...
    result = {"id": "BP-E253", "bars": {
        "B1_r0_lit": {"value": b1, "threshold": 0.80, "pass": p1},
        "B2_r1_lit": {"value": b2, "threshold": 0.80, "pass": p2},
        "B3_both": {"value": b3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E253"
