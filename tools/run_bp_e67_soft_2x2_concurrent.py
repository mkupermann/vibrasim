"""BP-E67 soft 2x2 concurrent dual-drive under identity. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((1971,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E67 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*90061+ti*449)
            w=World(cfg(seed)); train_all(w,rng)
            select_identity(w,rng)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0,L1])
            b1s.append(end_near(w,R0)>=1.0 and end_near(w,R1)>=1.0)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0])
            b2s.append(end_near(w,R0)>=1.0 and end_near(w,R1)<=0.25)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L1])
    ...
    result={"id":"BP-E67","bars":{
        "B1_concurrent_both":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_L0_only":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_L1_only":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E67"; out.mkdir(parents=True,exist_ok=True)
