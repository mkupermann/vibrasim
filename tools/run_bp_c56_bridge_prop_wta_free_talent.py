"""BP-C56 free dual bridge_atom_prop WTA vs G6 only. Headless."""
...
def cfg(seed, wta):
    return WorldConfig(
        ...
        bridge_atom_propagation_winner_take_all=bool(wta),
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 9401 + ti * 89 + int(treat) * 179)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5861,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C56 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C56"
