"""BP-C92 free dual emit_freq elevated vs default under n_emit=2. Headless."""
...
N_SIDE, T_FULL = 200, 400
SEEDS, TRIALS = (7461, 7471), 2
MID = 40.0
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        neuron_dynamics_enabled=True, theta_fire=2., t_refractory=0.02, n_emit=2,
        emit_freq=80000.0 if elev else 30000.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 20601 + ti * 317 + int(treat) * 421)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7461,), 1, 150) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C92 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C92"
