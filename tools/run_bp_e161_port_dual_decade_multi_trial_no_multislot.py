"""BP-E161 port dual decade multi-trial switch multislot OFF. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((4261,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E161 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 1971 + ti * 167)
            w = World(cfg(seed))
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E161"
