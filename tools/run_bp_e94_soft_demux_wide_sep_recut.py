"""BP-E94 soft DEMUX wide y-sep full restore + soft re-cut arm0. Headless."""
...
SEEDS, TRIALS = (2721, 2731), 6
N_WRITE, T_PROP, N_RES = 10, 45, 6
L = np.array([12., 29., 25.])
# wide y-sep: mids at 10, 32, 48 — min neighbor dist 18 > r=10
ARMS = []
for y in (10., 32., 48.):
    ARMS.append({
        "M": np.array([35., y, 25.]),
        "R": np.array([58., y, 25.]),
        "I": np.array([35., y + (2.0 if y < 30 else -2.0), 25.]),
...
def main(argv=None):
    ...
    seeds,trials=((2721,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E94 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*117157+ti*631)
            ...
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,L)
            r0off=end_near(w,ARMS[0]["R"])<=0.25
            r1on=end_near(w,ARMS[1]["R"])>=1.0
            r2on=end_near(w,ARMS[2]["R"])>=1.0
            b2s.append(r0off)
            b3s.append(r1on and r2on)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.80
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E94","bars":{
        "B1_all_on":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_r0_off":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_others_on":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E94"; out.mkdir(parents=True,exist_ok=True)
