"""BP-E23: no-scoring-table map; rewire control on modal gap. Headless."""
...
def score(routes: list[tuple[float, float]]) -> tuple[float, float]:
    ...
    if len(g0) == 0 or len(g1) == 0:
        ...
        if len(g1) == 0:
            return 0.0, 0.0
    m0, m1 = float(np.mean(g0)), float(np.mean(g1))
    ok = 0
    for fL, fR in routes:
        if fL <= med:
            ok += int(abs(fR - m0) <= abs(fR - m1))
        else:
            ok += int(abs(fR - m1) <= abs(fR - m0))
    cons = ok / len(routes)
...
def main(argv=None) -> int:
    ...
    seeds, trials = ((701,), 4) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E23 start smoke={args.smoke}")
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 41083 + ti * 167)
            ...
            train(w2, rng)
            rewire(w2, rng)
            _, gap2 = score(collect_routes(w2, bridged_L(w2)))
            b3s.append(gap2)
    a1, a2, a3 = float(np.mean(b1s)), float(np.mean(b2s)), float(np.mean(b3s))
    p1, p2, p3 = a1 >= 0.80, a2 >= 0.25, a3 <= 0.15
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {
        "id": "BP-E23",
        "bars": {
            "B1_self_cons": {"value": a1, "threshold": 0.80, "pass": p1},
            "B2_treat_gap": {"value": a2, "threshold": 0.25, "pass": p2},
            "B3_rewire_gap": {"value": a3, "threshold": 0.15, "pass": p3},
        },
        "verdict": verdict,
    }
    out = Path.home() / ".eqmod" / "bet" / "BP-E23"
