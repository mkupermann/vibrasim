"""BP-E111 soft 2x2 identity-diag cut then selective restore 00 only. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3141,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E111 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*134217+ti*751)
            w=World(cfg(seed)); train_all(w,rng)
            dual_cut_all(w)
            for k in ARMS:
                restore(w,rng,k)
            soft_cut(w,"00"); soft_cut(w,"11")  # identity diag → pure swap
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0])
            b1s.append(end_near(w,R1)>=1.0 and end_near(w,R0)<=0.25)
            restore(w,rng,"00")
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0])
    ...
    result={"id":"BP-E111","bars":{
        "B1_L0_swap":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_L0_fanout":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_L1_still_swap":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E111"; out.mkdir(parents=True,exist_ok=True)
