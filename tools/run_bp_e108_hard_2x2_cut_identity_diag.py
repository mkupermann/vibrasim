"""BP-E108 soft 2x2 full restore then hard-cut identity diagonal 00+11. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3061,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E108 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*131213+ti*727)
    ...
    result={"id":"BP-E108","bars":{
        "B1_concurrent":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_L0_swap":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_L1_swap":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E108"; out.mkdir(parents=True,exist_ok=True)
