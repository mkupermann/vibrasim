"""BP-E19: bridged-L probe + argmax-R charge partner. Headless."""
...
def argmax_R_freq(w: World, L_idx: int) -> float:
...
def main(argv=None) -> int:
    ...
    seeds, trials = ((601,), 4) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E19 start smoke={args.smoke} seeds={seeds} trials={trials}")

    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 36061 + ti * 139)
            ...
            if not Ls:
                ...
                pred = argmax_R_freq(w, Li)
                b1s.append(partner_ok(pred, true_partner(fL)))
            ...
            if not Ls2:
                ...
                pred2 = argmax_R_freq(w2, Li2)
                b2s.append(partner_ok(pred2, true_partner(fL2)))
    ...
    result = {
        "id": "BP-E19",
        "bars": {
            "B1_bridged_argmax": {"value": a1, "threshold": 0.80, "pass": p1},
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E19"
