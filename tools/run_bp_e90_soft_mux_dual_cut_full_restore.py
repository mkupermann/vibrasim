"""BP-E90 soft 3-path MUX dual-cut then full restore all paths. Headless."""
...
def all_on_isol(w):
    """Each L_j lights only R_j."""
    for j in range(3):
        w.k_charge[:]=0; w.k_latch[:]=0
        fire_phase(w,PATHS[j]["L"])
        if end_near(w,PATHS[j]["R"])<1.0:
            return False
        for k in range(3):
            if k!=j and end_near(w,PATHS[k]["R"])>0.25:
                return False
    return True
...
def main(argv=None):
    ...
    seeds,trials=((2641,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E90 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*113143+ti*607)
            w=World(cfg(seed)); train_all(w,rng)
            dual_cut_all(w)
            b1s.append(all_off(w))
            for k in range(3):
                restore(w,rng,k)
            # B2: all three paths ON (may share isolation check in B3)
            ons=[]
            for j in range(3):
                w.k_charge[:]=0; w.k_latch[:]=0
                fire_phase(w,PATHS[j]["L"])
