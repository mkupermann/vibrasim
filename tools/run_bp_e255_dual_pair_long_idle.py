"""BP-E255 dual pair selective recall after long idle. Headless."""
...
def main(argv=None):
    ...
    seeds, trials, t_idle = ((7681,), 2, 100) if args.smoke else (SEEDS, TRIALS, T_IDLE)
    print(f"BP-E255 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 22801 + ti * 283)
            w = World(cfg(seed))
            train_both(w, rng)
            idle(w, t_idle)
            clear_state(w)
    ...
    result = {"id": "BP-E255", "bars": {
        "B1_post_idle_p0": {"value": b1, "threshold": 0.80, "pass": p1},
        "B2_post_idle_p1": {"value": b2, "threshold": 0.80, "pass": p2},
        "B3_both": {"value": b3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E255"
