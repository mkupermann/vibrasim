"""BP-E120 hard dual-cut split R0a/R0b then selective soft restore 00. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3321,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E120 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*143217+ti*823)
            ...
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0])
            b2s.append(end_near(w,R0a)>=1.0 and end_near(w,R1)>=1.0)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L1])
            b3s.append(end_near(w,R0b)<=0.25 and end_near(w,R1)>=1.0)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.80
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E120","bars":{
        "B1_hard_split_silence":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_L0_R0a_fanout":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_L1_R0b_still_off":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E120"; out.mkdir(parents=True,exist_ok=True)
