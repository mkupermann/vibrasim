"""BP-C34 free dual elevated r_2 vs default. Headless."""
...
def cfg(seed, r2):
    return WorldConfig(
        ...
        rng_seed=seed, r_1=5., r_2=float(r2), freq_tolerance=0.03,
...
def run_one(seed, ti, treat, t_total):
    w = World(cfg(seed, 50.0 if treat else 28.0))
    rng = np.random.default_rng(seed * 4701 + ti * 67 + int(treat) * 61)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((4881,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C34 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C34"
