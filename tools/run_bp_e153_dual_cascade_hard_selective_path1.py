"""BP-E153 dual cascade AND hard selective silence path1. Headless."""
...
def path1_off(w):
    return peak_at(w,[L1a,L1b],R1)<=0.25

def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument("--smoke",action="store_true")
    args=p.parse_args(argv)
    seeds,trials=((4061,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E153 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*176217+ti*1049)
            w=World(cfg(seed)); train(w,rng)
            b1s.append(path0_dual_on(w) and path1_dual_on(w))
            hard_cut_at(w,I1a); hard_cut_at(w,I1b)
            b2s.append(path1_off(w) and path0_dual_on(w))
            restore_p1(w,rng)
    ...
    result={"id":"BP-E153","bars":{
        "B1_both_dual_on":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_p1_off_p0_on":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_restore_p1_both_on":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E153"; out.mkdir(parents=True,exist_ok=True)
