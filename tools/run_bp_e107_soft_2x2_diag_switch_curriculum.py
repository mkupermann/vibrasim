"""BP-E107 soft 2x2 multi-trial diagonal switch after full restore. Headless."""
...
def swap_ok(w):
    ...
    fire_phase(w,[L0])
    a=end_near(w,R1)>=1.0 and end_near(w,R0)<=0.25
    w.k_charge[:]=0; w.k_latch[:]=0
    fire_phase(w,[L1])
    b=end_near(w,R0)>=1.0 and end_near(w,R1)<=0.25
    return a and b

def identity_ok(w):
    w.k_charge[:]=0; w.k_latch[:]=0
    fire_phase(w,[L0])
    a=end_near(w,R0)>=1.0 and end_near(w,R1)<=0.25
    w.k_charge[:]=0; w.k_latch[:]=0
    fire_phase(w,[L1])
    b=end_near(w,R1)>=1.0 and end_near(w,R0)<=0.25
    return a and b

def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument("--smoke",action="store_true")
    args=p.parse_args(argv)
    seeds,trials=((3041,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E107 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*130211+ti*719)
            w=World(cfg(seed)); train_all(w,rng)
            dual_cut_all(w)
            for k in ARMS:
                restore(w,rng,k)
            soft_cut(w,"00"); soft_cut(w,"11")
            b1s.append(swap_ok(w))
            soft_cut(w,"01"); soft_cut(w,"10")
            restore(w,rng,"00"); restore(w,rng,"11")
            b2s.append(identity_ok(w))
            soft_cut(w,"00"); soft_cut(w,"11")
            restore(w,rng,"01"); restore(w,rng,"10")
            b3s.append(swap_ok(w))
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.75
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E107","bars":{
        "B1_swap":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_identity":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_swap_again":{"value":a3,"threshold":0.75,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E107"; out.mkdir(parents=True,exist_ok=True)
