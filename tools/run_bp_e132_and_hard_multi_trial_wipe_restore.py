"""BP-E132 coincidence AND multi-trial hard wipe-restore cycle. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3601,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E132 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*155217+ti*907)
    ...
    result={"id":"BP-E132","bars":{
        "B1_first_silence":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_first_restore":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_second_restore":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E132"; out.mkdir(parents=True,exist_ok=True)
