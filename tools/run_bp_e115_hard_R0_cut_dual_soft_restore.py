"""BP-E115 hard dual-cut R0 then dual soft restore 00+10. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3221,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E115 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*138217+ti*787)
    ...
    result={"id":"BP-E115","bars":{
        "B1_hard_R0_silent":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_L0_fanout":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_L1_fanout":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E115"; out.mkdir(parents=True,exist_ok=True)
