"""BP-E77 dual soft-cut then full restore AND + OR. Headless."""
...
SEEDS, TRIALS = (2231, 2241), 8
N_WRITE, T_PROP, N_RES = 12, 80, 8
L1=np.array([12.,18.,25.]); L2=np.array([12.,32.,25.])
L3=np.array([12.,25.,40.])
M=np.array([40.,25.,25.]); R=np.array([68.,25.,25.])
I1=np.array([12.,18.,25.]); I3=np.array([12.,25.,40.])
...
def train(w,rng):
    ...
    for i in range(w.k_count):
        ...
        if w.k_alive[i] and int(w.k_level[i])>=4:
            if float(np.linalg.norm(w.k_pos[i]-I1))<=5.5 or float(np.linalg.norm(w.k_pos[i]-I3))<=5.5:
                w.k_weaken_bridge_emitter[i]=1

def restore_full(w,rng):
    link(w,rng,L1,M,400.,1500.,N_RES)
    link(w,rng,L3,R,600.,5000.,N_RES)
...
def main(argv=None):
    ...
    seeds,trials=((2231,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E77 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*100093+ti*521)
            w=World(cfg(seed)); train(w,rng)
            soft_cut_at(w,I1); soft_cut_at(w,I3)
            b1s.append(peak_R(w,[L1,L2])<=0.25 and peak_R(w,[L3])<=0.25)
            disarm(w)
            restore_full(w,rng)
            b2s.append(peak_R(w,[L1,L2])>=1.0 and peak_R(w,[L3])>=1.0)
            b3s.append(peak_R(w,[L1])<=0.25)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.90,a2>=0.85,a3>=0.90
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E77","bars":{
        "B1_both_off":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_both_restore":{"value":a2,"threshold":0.85,"pass":p2},
        "B3_L1_only_off":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E77"; out.mkdir(parents=True,exist_ok=True)
