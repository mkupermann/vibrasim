"""BP-E180 split-port arm-selective soft weaken R0; c1 survives. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((4781,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E180 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 4201 + ti * 89)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_near(w, L0)
    ...
    result = {"id": "BP-E180", "bars": {
        "B1_pre_c0_select": {"value": a1, "threshold": 0.90, "pass": p1},
        "B2_post_soft_c0_fail": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_c1_survives": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E180"
