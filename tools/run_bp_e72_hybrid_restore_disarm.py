"""BP-E72 soft-cut OR bypass, disarm weaken emitters, restore L3-R. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((2131,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E72 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*95079+ti*487)
            w=World(cfg(seed)); train(w,rng)
            soft_cut_L3(w)
            b1s.append(peak_R(w,[L3])<=0.25)
            disarm_weaken(w)
            restore_L3R(w,rng)
    ...
    result={"id":"BP-E72","bars":{
        "B1_bypass_off":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_disarm_restore":{"value":a2,"threshold":0.85,"pass":p2},
        "B3_and_on":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E72"; out.mkdir(parents=True,exist_ok=True)
