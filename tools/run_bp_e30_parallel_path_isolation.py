"""BP-E30 parallel two-chain path isolation. Headless."""
...
def make_cfg(seed):
    return WorldConfig(
        ...
        ilw_pair_link_enabled=True, ilw_pair_link_delta=1., ilw_pair_replace_enabled=True,
...
def main(argv=None):
    ...
    seeds,trials=((981,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E30 start smoke={args.smoke}")
    b1s,b2s,b3s,b4s=[],[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*49013+ti*211)
    ...
    out=Path.home()/".eqmod"/"bet"/"BP-E30"; out.mkdir(parents=True, exist_ok=True)
