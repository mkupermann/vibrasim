"""BP-C21 free dual talent with atom_valence=2 vs unlimited control. Headless."""
...
SEEDS, TRIALS = (2861, 2871, 2881), 3
MID = 40.0
LOW, HIGH = (100.0, 2000.0), (500.0, 10000.0)

def cfg(seed, valence):
    return WorldConfig(
...
def run_one(seed, ti, valence, t_total):
    w=World(cfg(seed, valence))
    rng=np.random.default_rng(seed*1301+ti*101+int(valence)*7)
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((2861,),1,300) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C21 start smoke={args.smoke}")
    on,off=[],[]
    for s in seeds:
        for ti in range(trials):
            on.append(run_one(s,ti,2,t_tot))
            off.append(run_one(s,ti,0,t_tot))
    ...
    result={"id":"BP-C21","bars":{
        "B1_valence_spec":{"value":b1,"threshold":0.90,"pass":p1},
        "B2_ctrl_spec":{"value":b2,"threshold":0.80,"pass":p2},
        "B3_valence_pop":{"value":b3,"threshold":0.80,"pass":p3},
        "B4_delta":{"value":b4,"threshold":0.15,"pass":p4},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-C21"; out.mkdir(parents=True,exist_ok=True)
