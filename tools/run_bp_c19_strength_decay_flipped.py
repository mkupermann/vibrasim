"""BP-C19 strength-decay flipped dual-band reverse specialisation. Headless."""
...
def cfg(seed, decay):
    return WorldConfig(
        ...
        ilw_strength_decay_tau=float(TAU if decay else 0.0),
    )
...
def sides(w):
    ...
    # reverse: high on left -> mean_L > mean_R
    ok=pop and float(np.mean(L))>float(np.mean(R))
    return pop, ok

def run_one(seed, ti, decay, t_total):
    w=World(cfg(seed, decay))
    rng=np.random.default_rng(seed*2701+ti*89+int(decay)*37)
    # FLIPPED: left HIGH, right LOW
    inject(w,rng,N_SIDE,8,32,HIGH[0],HIGH[1])
    inject(w,rng,N_SIDE,48,72,LOW[0],LOW[1])
    evolve(w,t_total)
    return sides(w)
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((2361,),1,300) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C19 start smoke={args.smoke}")
    on,off=[],[]
    for s in seeds:
    ...
    b4=b1-b2
    p1,p2,p3,p4=b1>=0.80,b2<=0.70,b3>=0.80,b4>=0.10
    verdict="PASS" if all([p1,p2,p3,p4]) else "NULL"
    result={"id":"BP-C19","bars":{
        "B1_decay_reverse":{"value":b1,"threshold":0.80,"pass":p1},
        "B2_ctrl_reverse":{"value":b2,"threshold":0.70,"pass":p2},
        "B3_decay_pop":{"value":b3,"threshold":0.80,"pass":p3},
        "B4_delta":{"value":b4,"threshold":0.10,"pass":p4},
    },"verdict":verdict,"tau":TAU,"orientation":"L_high_R_low"}
    out=Path.home()/".eqmod"/"bet"/"BP-C19"; out.mkdir(parents=True,exist_ok=True)
