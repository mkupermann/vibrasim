"""BP-E128 coincidence AND soft dual wipe both L, full restore, dual fire ON. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3521,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E128 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*151217+ti*877)
    ...
    result={"id":"BP-E128","bars":{
        "B1_and_initial":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_wipe_silence":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_restore_dual":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E128"; out.mkdir(parents=True,exist_ok=True)
