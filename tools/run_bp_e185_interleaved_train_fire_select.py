"""BP-E185 interleaved multi-trial train fire-select. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((4981,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E185 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 5201 + ti * 43)
            w = World(cfg(seed))
            train_interleaved(w, rng)
            clear_state(w)
            fire_L_band(w, F_LO)
            ok_a = select_lo(w)
            b1s.append(ok_a)
    ...
    result = {"id": "BP-E185", "bars": {
        "B1_Llo_select": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_Lhi_select": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_both": {"value": a3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E185"
