"""BP-E122 hard split R0 multi-trial selective silence-restore-recut 00. Headless."""
...
SEEDS, TRIALS = (3361, 3371), 6
N_WRITE, T_PROP, N_RES = 10, 45, 6
L0=np.array([12.,12.,25.]); L1=np.array([12.,48.,25.])
R0a=np.array([58.,12.,25.]); R0b=np.array([58.,48.,25.]); R1=np.array([58.,30.,25.])
ARMS = {
    "00": {"M": np.array([35.,12.,25.]), "I": np.array([35.,12.,30.]), "L": L0, "R": R0a, "fM": 1200., "fR": 4000.},
    "01": {"M": np.array([35.,20.,18.]), "I": np.array([35.,20.,13.]), "L": L0, "R": R1, "fM": 1400., "fR": 4500.},
    "10": {"M": np.array([35.,48.,25.]), "I": np.array([35.,48.,30.]), "L": L1, "R": R0b, "fM": 1600., "fR": 5000.},
    "11": {"M": np.array([35.,40.,18.]), "I": np.array([35.,40.,13.]), "L": L1, "R": R1, "fM": 1800., "fR": 5500.},
}
fL = 400.
...
def main(argv=None):
    ...
    seeds,trials=((3361,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E122 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*145217+ti*829)
            w=World(cfg(seed)); train_all(w,rng)
            dual_cut_all(w)
            for k in ARMS:
                restore(w,rng,k)
            hard_cut(w,"00"); hard_cut(w,"10")
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0])
            ok0 = end_near(w,R0a)<=0.25 and end_near(w,R1)>=1.0
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L1])
            ok1 = end_near(w,R0b)<=0.25 and end_near(w,R1)>=1.0
            b1s.append(ok0 and ok1)
            restore(w,rng,"00")
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0])
            r0 = end_near(w,R0a)>=1.0 and end_near(w,R1)>=1.0
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L1])
            r1 = end_near(w,R0b)<=0.25 and end_near(w,R1)>=1.0
            b2s.append(r0 and r1)
            hard_cut(w,"00")
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0])
            s0 = end_near(w,R0a)<=0.25 and end_near(w,R1)>=1.0
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L1])
            s1 = end_near(w,R0b)<=0.25 and end_near(w,R1)>=1.0
            b3s.append(s0 and s1)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.80
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E122","bars":{
        "B1_hard_dual_silence":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_selective_restore":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_hard_selective_recut":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E122"; out.mkdir(parents=True,exist_ok=True)
