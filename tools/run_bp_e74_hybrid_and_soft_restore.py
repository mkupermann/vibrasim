"""BP-E74 soft-disable AND on hybrid + disarm restore L1-M. Headless."""
...
def train(w,rng):
    ...
    for i in range(w.k_count):
        ...
        if w.k_alive[i] and int(w.k_level[i])>=4 and float(np.linalg.norm(w.k_pos[i]-I))<=5.5:
...
def main(argv=None):
    ...
    seeds,trials=((2171,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E74 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*97087+ti*499)
            ...
            b3s.append(peak_R(w,[L3])>=1.0)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.90,a2>=0.85,a3>=0.90
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E74","bars":{
        "B1_and_off":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_and_restore":{"value":a2,"threshold":0.85,"pass":p2},
        "B3_bypass_on":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E74"; out.mkdir(parents=True,exist_ok=True)
