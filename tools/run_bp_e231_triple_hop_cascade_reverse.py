"""BP-E231 triple-hop cascade reverse L-M-A-R fire-select. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((6921,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E231 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 15201 + ti * 149)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_near(w, R0)
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E231"
