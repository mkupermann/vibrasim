"""BP-C73 free dual r_strengthen elevated vs default. Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        r_strengthen=12.0 if elev else 5.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 12601 + ti * 179 + int(treat) * 281)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6531,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C73 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C73"
