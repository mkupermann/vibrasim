"""BP-C13 free dual talent with dream phase vs wake-only control. Headless."""
...
N_SIDE, T_WAKE, T_DREAM = 400, 900, 300
SEEDS, TRIALS = (1881, 1891, 1901), 3
MID = 40.0
...
def cfg(seed):
    return WorldConfig(
        ...
        btsp_enabled=True, btsp_tau_eligibility=6.0,  # eligibility seeds for dream replay
        dream_mode_enabled=False,  # toggled mid-run for treatment
    )
...
def run_one(seed, ti, use_dream, t_wake, t_dream):
    w=World(cfg(seed))
    rng=np.random.default_rng(seed*2107+ti*61+int(use_dream)*13)
    inject(w,rng,N_SIDE,8,32,LOW[0],LOW[1])
    inject(w,rng,N_SIDE,48,72,HIGH[0],HIGH[1])
    if use_dream:
        evolve(w,t_wake,dream=False)
        evolve(w,t_dream,dream=True)
    else:
        evolve(w,t_wake+t_dream,dream=False)
    return sides(w)
...
def main(argv=None):
    ...
    if args.smoke:
        seeds,trials,tw,td=((1881,),1,200,50)
    else:
        seeds,trials,tw,td=SEEDS,TRIALS,T_WAKE,T_DREAM
    print(f"BP-C13 start smoke={args.smoke}")
    on,off=[],[]
    for s in seeds:
        for ti in range(trials):
            on.append(run_one(s,ti,True,tw,td))
            off.append(run_one(s,ti,False,tw,td))
    ...
    result={"id":"BP-C13","bars":{
        "B1_dream_spec":{"value":b1,"threshold":0.90,"pass":p1},
        "B2_ctrl_spec":{"value":b2,"threshold":0.80,"pass":p2},
        "B3_dream_pop":{"value":b3,"threshold":0.80,"pass":p3},
        "B4_delta":{"value":b4,"threshold":0.15,"pass":p4},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-C13"; out.mkdir(parents=True,exist_ok=True)
