"""BP-E43 retrain path after structural kill. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((1331,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E43 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*65073+ti*293)
    ...
    result={"id":"BP-E43","bars":{
        "B1_initial":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_after_cut":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_retrain":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E43"; out.mkdir(parents=True,exist_ok=True)
