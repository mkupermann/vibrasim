"""BP-C36 free dual tighter freq_tolerance vs default. Headless."""
...
def run_one(seed, ti, treat, t_total):
    w = World(cfg(seed, 0.01 if treat else 0.03))
    rng = np.random.default_rng(seed * 5101 + ti * 47 + int(treat) * 71)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((4961,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C36 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C36"
