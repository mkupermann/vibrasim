"""BP-E227 cascade reverse hard mid-kill M1; reverse p0 survives. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((6721,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E227 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 13501 + ti * 113)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_near(w, R1)
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E227"
