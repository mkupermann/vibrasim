"""BP-C20 strength-decay free dual with/without midplane wall. Headless."""
...
def cfg(seed, wall):
    return WorldConfig(
        ...
        midplane_wall_enabled=bool(wall), midplane_wall_x=MID,
...
def run_one(seed, ti, wall, t_total):
    w=World(cfg(seed, wall))
    rng=np.random.default_rng(seed*2803+ti*97+int(wall)*41)
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((2391,),1,300) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C20 start smoke={args.smoke}")
    nowall,withwall=[],[]
    for s in seeds:
        for ti in range(trials):
            nowall.append(run_one(s,ti,False,t_tot))
            withwall.append(run_one(s,ti,True,t_tot))
    b1=float(np.mean([1 if r[1] else 0 for r in nowall]))
    b2=float(np.mean([1 if r[1] else 0 for r in withwall]))
    b3=float(np.mean([1 if r[0] else 0 for r in nowall]))
    p1,p2,p3=b1<=0.70,b2>=0.70,b3>=0.70
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-C20","bars":{
        "B1_nowall_spec":{"value":b1,"threshold":0.70,"pass":p1},
        "B2_wall_spec":{"value":b2,"threshold":0.70,"pass":p2},
        "B3_nowall_pop":{"value":b3,"threshold":0.70,"pass":p3},
    },"verdict":verdict,"tau":TAU}
    out=Path.home()/".eqmod"/"bet"/"BP-C20"; out.mkdir(parents=True,exist_ok=True)
