"""BP-E192 content cascade fire-select with graded bridge_prop_min_strength. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((5201,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E192 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 6301 + ti * 5)
            w = World(cfg(seed))
            train_both(w, rng)
            # probe under graded prop gate
            w.config = replace(w.config, bridge_prop_min_strength=MIN_STR)
            clear_state(w)
            fire_near(w, L0)
            ok0 = select_p0(w)
            b1s.append(ok0)
    ...
    result = {"id": "BP-E192", "bars": {
        "B1_path0_graded": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_path1_graded": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_both": {"value": a3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E192"
