"""BP-E196 train-time pattern_id via active_pattern_id no post-hoc tag. Headless."""
...
def tag_rate_L_lo_pid1(w):
    ...
    for i in range(w.k_count):
        ...
        if float(w.k_freq[i]) >= F_MID:
            continue
        n += 1
        if int(w.k_pattern_id[i]) == 1:
            n1 += 1
    return n >= 1 and (n1 / n) >= 0.5
...
def main(argv=None):
    ...
    seeds, trials = ((5421,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E196 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 7201 + ti * 29)
            w = World(cfg(seed))
            train_both_with_pid(w, rng)
            b3s.append(tag_rate_L_lo_pid1(w))
            w.active_pattern_id = 1
            clear_state(w)
            fire_L_band(w, F_LO)
    ...
    result = {"id": "BP-E196", "bars": {
        "B1_pid1_select": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_pid2_select": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_train_tag_rate": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E196"
