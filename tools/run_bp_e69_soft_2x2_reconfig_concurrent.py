"""BP-E69 soft 2x2 reconfigure identity/swap/identity with concurrent probes. Headless."""
...
def concurrent_both_on(w):
    w.k_charge[:]=0; w.k_latch[:]=0
    fire_phase(w,[L0,L1])
    return end_near(w,R0)>=1.0 and end_near(w,R1)>=1.0

def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument("--smoke",action="store_true")
    args=p.parse_args(argv)
    seeds,trials=((2041,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E69 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*92067+ti*461)
            w=World(cfg(seed)); train_all(w,rng)
            select_keep(w,rng,{"00","11"})
            b1s.append(concurrent_both_on(w))
            select_keep(w,rng,{"01","10"})
            b2s.append(concurrent_both_on(w))
            select_keep(w,rng,{"00","11"})
            b3s.append(concurrent_both_on(w))
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.75
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E69","bars":{
        "B1_id_concurrent":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_swap_concurrent":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_reid_concurrent":{"value":a3,"threshold":0.75,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E69"; out.mkdir(parents=True,exist_ok=True)
