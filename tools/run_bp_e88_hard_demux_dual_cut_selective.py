"""BP-E88 hard DEMUX dual-kill all then selective arm restore. Headless."""
...
def probe_only(w, k):
    ...
    fire_phase(w,L)
    ok=end_near(w,ARMS[k]["R"])>=1.0
    others=all(end_near(w,ARMS[j]["R"])<=0.25 for j in range(3) if j!=k)
    return ok and others
...
def main(argv=None):
    ...
    seeds,trials=((2601,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E88 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*111139+ti*599)
