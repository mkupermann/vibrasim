"""BP-C33 free dual PRIM7 midplane sideband cull vs off. Headless."""
...
SEEDS, TRIALS = (4841, 4851), 2
MID = 40.0
LOW, HIGH = (100.0, 2000.0), (500.0, 10000.0)
F_MID = math.sqrt(500.0 * 5000.0)
...
def cfg(seed, cull):
    return WorldConfig(
        ...
        midplane_sideband_cull_enabled=bool(cull),
        midplane_gate_f_mid=F_MID,
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 4501 + ti * 73 + int(treat) * 59)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((4841,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C33 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C33"
