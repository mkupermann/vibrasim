"""BP-E121 soft split R0 multi-trial selective silence-restore-recut 00. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3341,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E121 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*144217+ti*827)
            w=World(cfg(seed)); train_all(w,rng)
            dual_cut_all(w)
            for k in ARMS:
                restore(w,rng,k)
            soft_cut(w,"00"); soft_cut(w,"10")
    ...
    result={"id":"BP-E121","bars":{
        "B1_dual_silence":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_selective_restore":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_selective_recut":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E121"; out.mkdir(parents=True,exist_ok=True)
