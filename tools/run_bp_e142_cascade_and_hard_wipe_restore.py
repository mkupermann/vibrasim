"""BP-E142 cascade coincidence AND multi-hop hard dual wipe-restore. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3841,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E142 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*165217+ti*977)
    ...
    result={"id":"BP-E142","bars":{
        "B1_cascade_and":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_hard_wipe_silence":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_restore_dual":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E142"; out.mkdir(parents=True,exist_ok=True)
