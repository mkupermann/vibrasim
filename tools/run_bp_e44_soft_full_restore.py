"""BP-E44 soft weaken + full L-M and M-R restore. Headless."""
...
def train_path(w,rng,n=N_WRITE):
    for _ in range(n):
...
def main(argv=None):
    ...
    seeds,trials=((1371,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E44 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*67081+ti*311)
            w=World(cfg(seed)); train_path(w,rng); seed_I(w,rng)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,L); b1s.append(end_R(w)>=1.0)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,I); fire_phase(w,L); b2s.append(end_R(w)<=0.25)
            train_path(w,rng,N_RESTORE)
            w.k_charge[:]=0; w.k_latch[:]=0
    ...
    result={"id":"BP-E44","bars":{
        "B1_on":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_off":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_full_restore":{"value":a3,"threshold":0.85,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E44"; out.mkdir(parents=True,exist_ok=True)
