"""BP-C96 free dual btsp_excitability_bias elevated vs 0 (BTSP ON both). Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        btsp_enabled=True,
        btsp_excitability_bias=2.0 if elev else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 22201 + ti * 353 + int(treat) * 449)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7621,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C96 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C96"
