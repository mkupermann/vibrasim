"""BP-C11 free dual talent with BTSP vs no-BTSP control. Headless."""
...
def cfg(seed, btsp):
    return WorldConfig(
        ...
        btsp_enabled=bool(btsp),
        btsp_tau_eligibility=6.0, btsp_plateau_charge_threshold=5.0,
        btsp_potentiation=50.0, btsp_radius=30.0,
    )
...
def run_one(seed, ti, btsp, t_total):
    w=World(cfg(seed, btsp))
    rng=np.random.default_rng(seed*1901+ti*53+int(btsp)*7)
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((1741,),1,300) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C11 start smoke={args.smoke}")
    on,off=[],[]
    for s in seeds:
    ...
    result={"id":"BP-C11","bars":{
        "B1_btsp_spec":{"value":b1,"threshold":0.90,"pass":p1},
        "B2_ctrl_spec":{"value":b2,"threshold":0.80,"pass":p2},
        "B3_btsp_pop":{"value":b3,"threshold":0.80,"pass":p3},
        "B4_delta":{"value":b4,"threshold":0.15,"pass":p4},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-C11"; out.mkdir(parents=True,exist_ok=True)
