"""BP-E151 dual cascade AND hard wipe both paths then full restore both. Headless."""
...
def restore_both(w,rng):
    ...
    link(w,rng,L1a,M1,400.,1400.,N_RES); link(w,rng,L1b,M1,800.,1400.,N_RES)
    link(w,rng,M1,A1,1400.,3200.,N_RES); link(w,rng,A1,R1,3200.,6200.,N_RES)
    arm_gate_at(w,M0); arm_gate_at(w,M1)
...
def both_off(w):
    return peak_at(w,[L0a,L0b],R0)<=0.25 and peak_at(w,[L1a,L1b],R1)<=0.25

def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument("--smoke",action="store_true")
    args=p.parse_args(argv)
    seeds,trials=((4021,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E151 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*174217+ti*1033)
            w=World(cfg(seed)); train(w,rng)
            b1s.append(path0_dual_on(w) and path1_dual_on(w))
            for pos in (I0a,I0b,I1a,I1b):
                hard_cut_at(w,pos)
            b2s.append(both_off(w))
            restore_both(w,rng)
    ...
    result={"id":"BP-E151","bars":{
        "B1_both_dual_on":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_both_dual_off":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_restore_both_on":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E151"; out.mkdir(parents=True,exist_ok=True)
