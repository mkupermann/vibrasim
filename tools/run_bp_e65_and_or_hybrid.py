"""BP-E65 (L1 AND L2) OR L3 hybrid path. Headless."""
...
def train(w,rng):
    ...
    link(w,rng,L3,R,600.,5000.)  # OR bypass
...
def main(argv=None):
    ...
    seeds,trials=((1931,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E65 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*88057+ti*439)
    ...
    result={"id":"BP-E65","bars":{
        "B1_and_on":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_or_bypass":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_L1_only_off":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E65"; out.mkdir(parents=True,exist_ok=True)
