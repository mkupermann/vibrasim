"""BP-C70 free dual fusion_bond_block vs off. Headless."""
...
def cfg(seed, block):
    return WorldConfig(
        ...
        fusion_bond_block=2 if block else 0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 12001 + ti * 157 + int(treat) * 263)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6411,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C70 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C70"
