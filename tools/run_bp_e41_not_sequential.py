"""BP-E41 sequential NOT: L lights R, then I clears. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((1271,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E41 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*62061+ti*277)
            ...
            fire_phase(w2,L); fire_phase(w2,I); b2s.append(end_R(w2)<=0.25)
            w3=World(cfg(seed)); train(w3,rng)
            w3.k_charge[:]=0; w3.k_latch[:]=0
            fire_phase(w3,I); b3s.append(end_R(w3)<=0.25)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.90,a2>=0.90,a3>=0.90
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E41","bars":{
        "B1_L_on":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_L_then_I":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_I_only":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E41"; out.mkdir(parents=True,exist_ok=True)
