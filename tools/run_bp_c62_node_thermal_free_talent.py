"""BP-C62 free dual node_thermal_speed vs off. Headless."""
...
SEEDS, TRIALS = (6101, 6111), 2
MID = 40.0
LOW, HIGH = (100.0, 2000.0), (500.0, 10000.0)

def cfg(seed, thermal):
    return WorldConfig(
        ...
        node_thermal_speed=2.0 if thermal else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 10601 + ti * 109 + int(treat) * 211)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6101,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C62 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C62"
