"""BP-E234 cascade reverse sequential retrain coexistence (no kill). Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((7041,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E234 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 16401 + ti * 163)
            w = World(cfg(seed))
            train_p0(w, rng)
            clear_state(w)
            fire_near(w, R0)
            b1s.append(reverse_p0(w))
            # retrain path1 only — no kill
            train_p1(w, rng)
            clear_state(w)
            fire_near(w, R1)
