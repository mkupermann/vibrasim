"""BP-E184 fire-select durability after long idle no retrain. Headless."""
...
def main(argv=None):
    ...
    seeds, trials, t_idle = ((4941,), 2, 100) if args.smoke else (SEEDS, TRIALS, T_IDLE)
    print(f"BP-E184 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 5001 + ti * 53)
            w = World(cfg(seed))
            train_both(w, rng)
            idle(w, t_idle)
            clear_state(w)
    ...
    result = {"id": "BP-E184", "bars": {
        "B1_Llo_after_idle": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_Lhi_after_idle": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_both": {"value": a3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E184"
