"""BP-E217 cascade reverse soft mid-kill M0; reverse p1 survives. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((6241,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E217 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 11301 + ti * 83)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_near(w, R0)
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E217"
