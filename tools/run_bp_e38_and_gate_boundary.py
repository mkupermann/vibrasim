"""BP-E38 AND-gate boundary (expected NULL vs E34 OR). Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((1141,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E38 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*57041+ti*251)
            w=World(cfg(seed)); train(w,rng)
            b1s.append(peak_R(w,[L1])<=0.25)
            w2=World(cfg(seed)); train(w2,rng)
            b2s.append(peak_R(w2,[L2])<=0.25)
            w3=World(cfg(seed)); train(w3,rng)
            b3s.append(peak_R(w3,[L1,L2])>=1.0)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.85,a2>=0.85,a3>=0.85
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E38","bars":{
        "B1_L1_silent":{"value":a1,"threshold":0.85,"pass":p1},
        "B2_L2_silent":{"value":a2,"threshold":0.85,"pass":p2},
        "B3_both_on":{"value":a3,"threshold":0.85,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E38"; out.mkdir(parents=True,exist_ok=True)
