"""BP-E116 soft 2x2 swap-diag cut then dual restore 01+10. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3241,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E116 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*139217+ti*797)
            w=World(cfg(seed)); train_all(w,rng)
            dual_cut_all(w)
            for k in ARMS:
                restore(w,rng,k)
            soft_cut(w,"01"); soft_cut(w,"10")  # swap diag → pure identity
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0])
            b1s.append(end_near(w,R0)>=1.0 and end_near(w,R1)<=0.25)
    ...
    result={"id":"BP-E116","bars":{
        "B1_L0_identity":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_L0_fanout":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_L1_fanout":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E116"; out.mkdir(parents=True,exist_ok=True)
