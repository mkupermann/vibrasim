"""BP-C42 free dual wider freq_tolerance vs default. Headless."""
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 6501 + ti * 2 + int(treat) * 101)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5241,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C42 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C42"
