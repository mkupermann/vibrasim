"""BP-E251 concurrent mixed fwd/rev triple-path drive. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((7521,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E251 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 21201 + ti * 257)
            w = World(cfg(seed))
            train_all(w, rng)
            clear_state(w)
            # fire L0 (fwd), R1 (rev), L2 (fwd)
            fire_near_all(w, [L[0], R[1], L[2]])
            t0 = peak_near(w, R[0]) >= 1.0
            t1 = peak_near(w, L[1]) >= 1.0
            t2 = peak_near(w, R[2]) >= 1.0
            b1s.append(t0)
            b2s.append(t1)
            b3s.append(t0 and t1 and t2)
    b1 = float(np.mean(b1s)); b2 = float(np.mean(b2s)); b3 = float(np.mean(b3s))
    p1, p2, p3 = b1 >= 0.80, b2 >= 0.80, b3 >= 0.70
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E251", "bars": {
        "B1_fwd_r0": {"value": b1, "threshold": 0.80, "pass": p1},
        "B2_rev_l1": {"value": b2, "threshold": 0.80, "pass": p2},
        "B3_all_three": {"value": b3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E251"
