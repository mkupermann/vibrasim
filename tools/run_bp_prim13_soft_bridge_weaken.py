"""PRIM13-D0 soft reversible bridge weaken NOT. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((1351,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"PRIM13-D0 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*66079+ti*307)
            w=World(cfg(seed)); train_path(w,rng); seed_I(w,rng)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,L); b1s.append(end_R(w)>=1.0)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,I); fire_phase(w,L)
            b2s.append(end_R(w)<=0.25)
            restore_MR(w,rng)
            w.k_charge[:]=0; w.k_latch[:]=0
    ...
    result={"id":"PRIM13-D0","bars":{
        "B1_on":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_weak_off":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_restore":{"value":a3,"threshold":0.85,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"PRIM13-D0"; out.mkdir(parents=True,exist_ok=True)
