"""BP-C75 free dual vibration_soft_cap vs off. Headless."""
...
def cfg(seed, soft):
    return WorldConfig(
        ...
        vibration_soft_cap=256 if soft else 0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 13001 + ti * 197 + int(treat) * 307)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6621,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C75 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C75"
