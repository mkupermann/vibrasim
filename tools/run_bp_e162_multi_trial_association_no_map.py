"""BP-E162 multi-trial dual-port association residual without baked map. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((4341,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E162 start smoke={args.smoke}", flush=True)
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            # treatment: multi-trial dual train then L-only
            rng=np.random.default_rng(seed*2101+ti*179)
            w=World(cfg(seed))
            ...
            # control: L-only no train
            rng2=np.random.default_rng(seed*2101+ti*179+99)
            w2=World(cfg(seed+7))
    ...
    # B2: control R-partner rate should be low; we store "no partner" success rate
    # Convert: b2 was "success if NO partner"; bar is ctrl partner ≤0.20 → no-partner ≥0.80
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.90
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E162","bars":{
        "B1_treat_R_partner":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_ctrl_no_R_partner":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_treat_L_low":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E162"; out.mkdir(parents=True,exist_ok=True)
