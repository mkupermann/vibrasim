"""BP-E49 three-path soft MUX curriculum. Headless."""
...
def probe(w,k):
    ...
    return end_near(w,PATHS[k]["R"])

def select_only(w,rng,k):
    # restore all then cut others
    for j in range(3):
        restore(w,rng,j)
    for j in range(3):
        if j==k: continue
        set_emitter(w,j)
        fire_phase(w,PATHS[j]["I"], n=30)
        set_emitter(w,k)  # clear others by re-set only k (zero then set k)
    # ensure only k uncut: restore k last
    restore(w,rng,k)

def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument("--smoke",action="store_true")
    args=p.parse_args(argv)
    seeds,trials=((1501,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E49 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*72097+ti*347)
            w=World(cfg(seed)); train_all(w,rng)
            # select path 0 only
            select_only(w,rng,0)
            b1s.append(probe(w,0)>=1.0 and probe(w,1)<=0.25 and probe(w,2)<=0.25)
            select_only(w,rng,1)
            b2s.append(probe(w,1)>=1.0 and probe(w,0)<=0.25 and probe(w,2)<=0.25)
            select_only(w,rng,2)
            b3s.append(probe(w,2)>=1.0 and probe(w,0)<=0.25 and probe(w,1)<=0.25)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.80
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E49","bars":{
        "B1_sel0":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_sel1":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_sel2":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E49"; out.mkdir(parents=True,exist_ok=True)
