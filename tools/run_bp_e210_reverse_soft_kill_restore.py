"""BP-E210 reverse split soft-kill L1 then retrain restore. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((5961,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E210 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 9901 + ti * 61)
            w = World(cfg(seed))
            train_both(w, rng)
            soft_cut_at(w, L1)
            idle(w, 10)
    ...
    result = {"id": "BP-E210", "bars": {
        "B1_post_soft_rev_fail": {"value": a1, "threshold": 0.70, "pass": p1},
        "B2_rev_pid1_survives": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_restore_rev_pid2": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E210"
