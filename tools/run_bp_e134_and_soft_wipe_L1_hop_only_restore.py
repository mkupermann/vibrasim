"""BP-E134 coincidence AND soft dual wipe, restore L1-M only (no M-R), then L2. Headless."""
...
def restore_L1_hop_only(w,rng):
    # L1-M only — no M-R rewrite
    link(w,rng,L1,M,400.,1500.,N_RES)
    arm_gate(w)

def restore_L2_and_MR(w,rng):
    link(w,rng,L2,M,800.,1500.,N_RES)
    link(w,rng,M,R,1500.,5000.,N_RES)
    arm_gate(w)
...
def main(argv=None):
    ...
    seeds,trials=((3641,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E134 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*157217+ti*919)
            w=World(cfg(seed)); train(w,rng)
            soft_cut_at(w,I1); soft_cut_at(w,I2)
            b1s.append(peak_R(w,[L1,L2])<=0.25)
            restore_L1_hop_only(w,rng)
            b2s.append(peak_R(w,[L1,L2])<=0.25)
            restore_L2_and_MR(w,rng)
            b3s.append(peak_R(w,[L1,L2])>=1.0)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.80
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E134","bars":{
        "B1_dual_wipe_silence":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_L1_hop_only_still_off":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_full_dual_on":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E134"; out.mkdir(parents=True,exist_ok=True)
