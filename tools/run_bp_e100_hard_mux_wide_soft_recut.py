"""BP-E100 hard MUX dual-kill full restore then soft re-cut path0 wide-sep. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((2841,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E100 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*123179+ti*661)
            w=World(cfg(seed)); train_all(w,rng)
            dual_kill_all(w)
            for k in range(3):
                restore(w,rng,k)
            b1s.append(all(path_on(w,k) for k in range(3)))
            soft_cut(w,0)
            b2s.append(path_off(w,0))
            b3s.append(path_on(w,1) and path_on(w,2))
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.80
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E100","bars":{
        "B1_all_on":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_p0_off":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_others_on":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E100"; out.mkdir(parents=True,exist_ok=True)
