"""BP-C91 free dual speech_loop_strength on vs off. Headless."""
...
def cfg(seed, on):
    return WorldConfig(
        ...
        speech_loop_strength=1.0 if on else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 18201 + ti * 313 + int(treat) * 419)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7221,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C91 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C91"
