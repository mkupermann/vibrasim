"""BP-C84 free dual polarity_split biased vs default. Headless."""
...
def cfg(seed, treat):
    return WorldConfig(
        ...
        polarity_split=0.9 if treat else 0.5,
    )
...
def inject(w, rng, n, x0, x1, f0, f1):
    ...
    for k, i in enumerate(slots):
        ...
        # respect polarity_split when sampling inject polarity
        w.s_pol[i] = bool(rng.random() < float(w.config.polarity_split))
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 15401 + ti * 271 + int(treat) * 373)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6941,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C84 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C84"
