"""BP-E202 asymmetric R-only pattern tags G12 wrong-arm block. Headless."""
...
def tag_r_only(w):
    for i in range(w.k_count):
        ...
        if float(w.k_pos[i, 0]) < MID:
            continue
        f = float(w.k_freq[i])
        # c0 partner is R-hi → pid1; c1 partner R-lo → pid2
...
def main(argv=None):
    ...
    seeds, trials = ((5641,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E202 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 8301 + ti * 43)
            w = World(cfg(seed))
            train_both(w, rng)
            tag_r_only(w)
    ...
    p1, p2, p3 = a1 >= 0.90, a2 >= 0.70, a3 >= 0.70
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E202", "bars": {
        "B1_correct_R_only": {"value": a1, "threshold": 0.90, "pass": p1},
        "B2_wrong_arm_R_only": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_both_end_wrong_fail": {"value": a3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E202"
