"""BP-E61 coincidence-AND gated relay L-G-R. Headless."""
...
SEEDS, TRIALS = (1791, 1801), 8
N_WRITE, T_PROP = 10, 50
L=np.array([15.,25.,25.]); G=np.array([40.,25.,25.]); R=np.array([65.,25.,25.])
...
def train(w,rng):
    link(w,rng,L,G,400.,1000.); link(w,rng,G,R,1000.,2500.)
...
def main(argv=None):
    ...
    seeds,trials=((1791,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E61 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*84041+ti*419)
            w=World(cfg(seed)); train(w,rng)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_targets(w,[L]); b1s.append(end_R(w)<=0.25)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_targets(w,[L,G]); b2s.append(end_R(w)>=1.0)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_targets(w,[G]); b3s.append(end_R(w)<=0.25)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.90,a2>=0.90,a3>=0.90
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E61","bars":{
        "B1_L_only_off":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_L_and_G_on":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_G_only_off":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E61"; out.mkdir(parents=True,exist_ok=True)
