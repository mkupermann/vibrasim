...
if __name__ == "__main__":
    print(f"=== G128: store+recall ASCII chars in matter memory (2 rows = 2 nibbles/byte) ===", flush=True)
    ...
    print("\n--- VERDICT ---", flush=True)
    print(f"G128 chars recalled exactly (both seeds): {allok}", flush=True)
    print(("G128: PASS - the matter memory stores and recalls ASCII CHARACTERS (text in persistent matter, no LLM)"
           if allok else "G128: NULL/PARTIAL - some chars mis-recalled (see output)"), flush=True)
