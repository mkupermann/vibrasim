"""BP-E123 dual 3-hop soft wipe both, full restore, soft re-cut path0. Headless."""
...
SEEDS, TRIALS = (3381, 3391), 6
N_WRITE, T_PROP, N_RES = 10, 50, 8
# y-sep=14 > soft weaken r=8
...
def main(argv=None):
    ...
    seeds,trials=((3381,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E123 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*146217+ti*839)
    ...
    out=Path.home()/".eqmod"/"bet"/"BP-E123"; out.mkdir(parents=True,exist_ok=True)
