"""BP-C38 free dual corr_plasticity Hebbian vs off. Headless."""
...
def cfg(seed, corr):
    return WorldConfig(
        ...
        corr_plasticity_rate=float(corr), corr_potentiation=1.0,
    )
...
def run_one(seed, ti, treat, t_total):
    w = World(cfg(seed, 0.5 if treat else 0.0))
    rng = np.random.default_rng(seed * 5501 + ti * 31 + int(treat) * 79)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5041,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C38 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C38"
