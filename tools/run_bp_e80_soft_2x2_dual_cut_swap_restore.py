"""BP-E80 soft 2x2 dual-cut all arms then selective swap restore. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((2441,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E80 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*103103+ti*547)
    ...
    result={"id":"BP-E80","bars":{
        "B1_both_off":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_swap_restore":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_L0_to_R1":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E80"; out.mkdir(parents=True,exist_ok=True)
