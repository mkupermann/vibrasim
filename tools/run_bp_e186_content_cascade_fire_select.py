"""BP-E186 content+cascade multi-hop fire-select two paths. Headless."""
...
SEEDS, TRIALS = (5021, 5031), 8
N_TRAIN, N_WRITE, T_PROP = 12, 12, 100
NEAR = 7.0
# path0 low→high cascade; path1 high→low cascade
...
def main(argv=None):
    ...
    seeds, trials = ((5021,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E186 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 5401 + ti * 37)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_near(w, L0)
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E186"
