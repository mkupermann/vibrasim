"""BP-E260 write-order residual with pair-link ON. Headless."""
...
def run_arm(seed, ti, l_first):
    rng = np.random.default_rng(seed * 24801 + ti * 331 + int(l_first) * 17)
...
def main(argv=None):
    ...
    seeds, trials = ((7881,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E260 start smoke={args.smoke}", flush=True)
    a_hits, b_hits = [], []
    for seed in seeds:
        for ti in range(trials):
            a_hits.append(run_arm(seed, ti, True))
            b_hits.append(run_arm(seed, ti, False))
    b1 = float(np.mean(a_hits))
    b2 = float(np.mean(b_hits))
    b3 = abs(b1 - b2)
    p1, p2, p3 = b1 >= 0.80, b2 >= 0.80, b3 <= 0.25
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E260", "bars": {
        "B1_L_first_residual": {"value": b1, "threshold": 0.80, "pass": p1},
        "B2_R_first_residual": {"value": b2, "threshold": 0.80, "pass": p2},
        "B3_abs_delta": {"value": b3, "threshold": 0.25, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E260"
