"""BP-C59 free dual bond_turnover_rate vs off. Headless."""
...
def cfg(seed, turn):
    return WorldConfig(
        ...
        bond_turnover_rate=0.01 if turn else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 10001 + ti * 101 + int(treat) * 193)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5981,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C59 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C59"
