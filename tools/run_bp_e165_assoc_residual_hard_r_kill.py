"""BP-E165 association residual after hard R-port kill. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((4421,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E165 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 2401 + ti * 167)
            w = World(cfg(seed))
            ...
            rng2 = np.random.default_rng(seed * 2401 + ti * 167 + 61)
            w2 = World(cfg(seed + 17))
            hard_cut_port(w2, PORT_R)
            idle(w2, 20)
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E165"
