"""BP-E63 hard-kill one arm of coincidence AND at L1. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((1861,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E63 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*86047+ti*431)
            w=World(cfg(seed)); train(w,rng)
            b1s.append(peak_R(w,[L1,L2])>=1.0)
            thr=float(w.config.theta_fire); dt=float(w.config.dt)
            for t in range(30):
                if t%5==0:
                    for i in range(w.k_count):
    ...
    result={"id":"BP-E63","bars":{
        "B1_both_on":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_hard_off":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_restore_on":{"value":a3,"threshold":0.85,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E63"; out.mkdir(parents=True,exist_ok=True)
