"""BP-C49 free dual bridge_atom_propagation_enabled vs off. Headless."""
...
def cfg(seed, prop):
    return WorldConfig(
        ...
        bridge_atom_propagation_enabled=bool(prop),
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 8001 + ti * 59 + int(treat) * 137)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5581,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C49 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C49"
