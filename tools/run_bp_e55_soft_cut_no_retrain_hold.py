"""BP-E55 soft cut then idle hold without retrain — silence must persist. Headless."""
...
SEEDS, TRIALS = (1621, 1631), 8
N_WRITE, T_PROP, T_HOLD = 10, 50, 80
...
def cfg(seed):
    return WorldConfig(
        ...
        fire_weaken_bridge_radius=18.0, fire_weaken_bridge_frac=1.0,
    )
...
def train(w,rng):
    link(w,rng,L,M,400.,1000.); link(w,rng,M,R,1000.,2500.)
...
def main(argv=None):
    ...
    seeds,trials=((1621,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E55 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*78019+ti*379)
            ...
            # idle hold — no retrain
            idle(w,T_HOLD)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,L); b3s.append(end_R(w)<=0.25)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.90,a2>=0.90,a3>=0.90
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E55","bars":{
        "B1_on":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_cut_off":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_hold_still_off":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E55"; out.mkdir(parents=True,exist_ok=True)
