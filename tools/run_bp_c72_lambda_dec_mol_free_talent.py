"""BP-C72 free dual lambda_dec_mol vs off. Headless."""
...
def cfg(seed, dec):
    return WorldConfig(
        ...
        lambda_dec_mol=0.01 if dec else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 12401 + ti * 167 + int(treat) * 271)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6491,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C72 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C72"
