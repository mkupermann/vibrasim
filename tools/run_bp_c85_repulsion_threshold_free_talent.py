"""BP-C85 free dual repulsion_threshold_ratio low vs default. Headless."""
...
def cfg(seed, treat):
    return WorldConfig(
        ...
        repulsion_threshold_ratio=2.0 if treat else 1000.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 15801 + ti * 277 + int(treat) * 379)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6981,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C85 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C85"
