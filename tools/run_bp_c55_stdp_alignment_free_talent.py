"""BP-C55 free dual stdp_alignment_strict vs stdp-only. Headless."""
...
def cfg(seed, strict):
    return WorldConfig(
        ...
        stdp_enabled=True,
        stdp_alignment_strict_threshold=0.95 if strict else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 9201 + ti * 83 + int(treat) * 173)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5821,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C55 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C55"
