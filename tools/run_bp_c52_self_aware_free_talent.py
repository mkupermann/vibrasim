"""BP-C52 free dual self_aware_enabled vs off. Headless."""
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 8601 + ti * 71 + int(treat) * 157)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5701,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C52 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C52"
