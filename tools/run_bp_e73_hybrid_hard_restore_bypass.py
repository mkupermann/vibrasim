"""BP-E73 hard-kill OR bypass then restore L3-R. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((2151,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E73 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*96083+ti*491)
    ...
    result={"id":"BP-E73","bars":{
        "B1_bypass_off":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_hard_restore":{"value":a2,"threshold":0.85,"pass":p2},
        "B3_and_on":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E73"; out.mkdir(parents=True,exist_ok=True)
