"""BP-E137 hybrid hard dual wipe I1+I3 then selective AND-only restore. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3701,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E137 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*160217+ti*941)
    ...
    result={"id":"BP-E137","bars":{
        "B1_both_silent":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_and_restored":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_or_still_off":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E137"; out.mkdir(parents=True,exist_ok=True)
