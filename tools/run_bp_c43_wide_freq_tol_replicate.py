"""BP-C43 replicate C42 wider freq_tolerance free dual larger N. Headless."""
...
SEEDS, TRIALS = (5281, 5291, 5301, 5311, 5321), 3
MID = 40.0
...
def cfg(seed, ft):
    return WorldConfig(
        ...
        rng_seed=seed, r_1=5., r_2=28., freq_tolerance=float(ft),
...
def run_one(seed, ti, treat, t_total):
    w = World(cfg(seed, 0.08 if treat else 0.03))
    rng = np.random.default_rng(seed * 6701 + ti * 11 + int(treat) * 107)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5281,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C43 start smoke={args.smoke} n_seeds={len(seeds)} trials={trials}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    }, "n_treat": len(on), "n_ctrl": len(off), "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-C43"
    out.mkdir(parents=True, exist_ok=True)
    path = out / ("result_smoke.json" if args.smoke else "result.json")
    path.write_text(json.dumps(result, indent=2), encoding="utf-8")
    for k, v in result["bars"].items():
        print(f"  {k}: {v['value']:.4f} thr={v['threshold']} pass={v['pass']}", flush=True)
    print(f"  n_treat={len(on)} n_ctrl={len(off)}", flush=True)
    print(f"--- VERDICT ---\nBP-C43: {verdict}\nwrote {path}\nDONE", flush=True)
