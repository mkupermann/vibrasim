"""BP-C63 free dual edge_closure_k vs off. Headless."""
...
def cfg(seed, edge):
    return WorldConfig(
        ...
        edge_closure_k=5.0 if edge else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 10801 + ti * 113 + int(treat) * 223)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6141,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C63 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C63"
