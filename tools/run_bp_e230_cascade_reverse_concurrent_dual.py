"""BP-E230 cascade reverse concurrent dual R fire. Headless."""
...
def both_lit(w):
    return peak_near(w, L0) >= 1.0 and peak_near(w, L1) >= 1.0

def main(argv=None):
    p = argparse.ArgumentParser(); p.add_argument("--smoke", action="store_true")
    args = p.parse_args(argv)
    seeds, trials = ((6881,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E230 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 14801 + ti * 137)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_near(w, R0)
            ...
            clear_state(w)
            fire_near_both(w, R0, R1)
            b3s.append(both_lit(w))
    b1 = float(np.mean(b1s)); b2 = float(np.mean(b2s)); b3 = float(np.mean(b3s))
    p1, p2, p3 = b1 >= 0.90, b2 >= 0.90, b3 >= 0.70
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E230", "bars": {
        "B1_seq_rev_p0": {"value": b1, "threshold": 0.90, "pass": p1},
        "B2_seq_rev_p1": {"value": b2, "threshold": 0.90, "pass": p2},
        "B3_concurrent_both": {"value": b3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E230"
