"""BP-E35 diamond path redundancy. Headless."""
...
SEEDS, TRIALS = (1081, 1091), 8
N_WRITE, T_PROP = 12, 60
L=np.array([12.,25.,25.]); M1=np.array([40.,15.,25.]); M2=np.array([40.,35.,25.]); R=np.array([68.,25.,25.])
...
def train(w,rng):
    link(w,rng,L,M1,400.,1500.); link(w,rng,M1,R,1500.,5000.)
    link(w,rng,L,M2,400.,2500.); link(w,rng,M2,R,2500.,5000.)

def kill_bridges_touching(w, center, rad=8.):
    for b in range(w.b_count):
        if not w.b_alive[b]: continue
        i,j=int(w.b_atom_i[b]),int(w.b_atom_j[b])
        for a in (i,j):
            if w.k_alive[a] and float(np.linalg.norm(w.k_pos[a]-center))<=rad:
                # only kill if bridge is the outbound M→R (partner near R) or any touch M
                w.b_alive[b]=False
                break
...
def main(argv=None):
    ...
    seeds,trials=((1081,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E35 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*54029+ti*233)
            w=World(cfg(seed)); train(w,rng)
            b1s.append(peak_R(w)>=1.0)
            w2=World(cfg(seed)); train(w2,rng)
            kill_bridges_touching(w2,M1)
            b2s.append(peak_R(w2)>=1.0)
            w3=World(cfg(seed)); train(w3,rng)
            kill_bridges_touching(w3,M1); kill_bridges_touching(w3,M2)
            b3s.append(peak_R(w3)<=0.25)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.90,a2>=0.85,a3>=0.90
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E35","bars":{
        "B1_full":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_kill_M1":{"value":a2,"threshold":0.85,"pass":p2},
        "B3_kill_both":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E35"; out.mkdir(parents=True,exist_ok=True)
