"""BP-C48 free dual sparse_firing_enabled vs off. Headless."""
...
def cfg(seed, sparse):
    return WorldConfig(
        ...
        sparse_firing_enabled=bool(sparse), sparse_firing_top_k=3,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 7801 + ti * 53 + int(treat) * 131)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5541,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C48 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C48"
