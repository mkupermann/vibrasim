"""BP-E129 coincidence AND hard dual wipe both L, full restore, dual fire ON. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3541,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E129 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*152217+ti*881)
            w=World(cfg(seed)); train(w,rng)
            dual_on = peak_R(w,[L1,L2])>=1.0
            single_off = peak_R(w,[L1])<=0.25
            b1s.append(dual_on and single_off)
            hard_cut_at(w,I1); hard_cut_at(w,I2)
    ...
    result={"id":"BP-E129","bars":{
        "B1_and_initial":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_hard_wipe_silence":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_restore_dual":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E129"; out.mkdir(parents=True,exist_ok=True)
