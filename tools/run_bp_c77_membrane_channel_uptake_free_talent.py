"""BP-C77 free dual membrane_channel_uptake vs channel without uptake. Headless."""
...
def cfg(seed, uptake):
    return WorldConfig(
        ...
        membrane_channel_k=1.0,
        membrane_channel_mode="atom",
        membrane_channel_uptake=bool(uptake),
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 13401 + ti * 211 + int(treat) * 313)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6701,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C77 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C77"
