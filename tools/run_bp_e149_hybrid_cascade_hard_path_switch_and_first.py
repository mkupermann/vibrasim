"""BP-E149 hybrid cascade multi-trial hard path-switch AND-OR-AND. Headless."""
...
def restore_cascade_and(w,rng):
    link(w,rng,L1,M,400.,1500.,N_RES); link(w,rng,L2,M,800.,1500.,N_RES)
    link(w,rng,M,A,1500.,3000.,N_RES); link(w,rng,A,R,3000.,6000.,N_RES)
    arm_gate(w)
...
def cascade_and_only(w):
    return peak_R(w,[L1,L2])>=1.0 and peak_R(w,[L3])<=0.25

def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument("--smoke",action="store_true")
    args=p.parse_args(argv)
    seeds,trials=((3981,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E149 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*172217+ti*1021)
            w=World(cfg(seed)); train(w,rng)
            hard_cut_at(w,I3)
            restore_cascade_and(w,rng)
            b1s.append(cascade_and_only(w))
            hard_cut_at(w,I1)
            restore_or(w,rng)
            b2s.append(or_only(w))
            hard_cut_at(w,I3)
            restore_cascade_and(w,rng)
            b3s.append(cascade_and_only(w))
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.75
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E149","bars":{
        "B1_cascade_and_only":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_or_only":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_cascade_and_only_again":{"value":a3,"threshold":0.75,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E149"; out.mkdir(parents=True,exist_ok=True)
