"""BP-E98 soft MUX full restore then hard re-cut path0. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((2801,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E98 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*121169+ti*653)
    ...
    out=Path.home()/".eqmod"/"bet"/"BP-E98"; out.mkdir(parents=True,exist_ok=True)
