"""BP-E174 fire selective residual soft R bridge weaken then restore. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((4621,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E174 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 3401 + ti * 123)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_L_band(w, F_LO)
    ...
    result = {"id": "BP-E174", "bars": {
        "B1_pre_select": {"value": a1, "threshold": 0.90, "pass": p1},
        "B2_post_weaken_fail": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_restore_select": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E174"
