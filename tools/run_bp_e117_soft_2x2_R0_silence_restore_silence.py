"""BP-E117 soft 2x2 multi-trial R0 silence, dual restore, silence again. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3261,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E117 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*140217+ti*809)
    ...
    result={"id":"BP-E117","bars":{
        "B1_first_silence":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_dual_restore":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_second_silence":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E117"; out.mkdir(parents=True,exist_ok=True)
