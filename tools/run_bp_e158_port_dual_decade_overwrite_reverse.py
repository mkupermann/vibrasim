"""BP-E158 port dual decade overwrite reverse last write wins. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((4201,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E158 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 1941 + ti * 151)
            w = World(cfg(seed))
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E158"
