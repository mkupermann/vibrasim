...
CELLS = [7.0, 14.0, 21.0]     # K=3 cells, WIDE pitch 7, tight radius (G118 fix: spacing)
K = len(CELLS)
CELLR = 1.5
NPAT = 3
...
if __name__ == "__main__":
    print(f"=== G119: WIDE-SPACED multi-bit matter memory (K={K} cells, pitch 7, {NPAT} patterns) ===", flush=True)
    seeds = [42, 7]
    R = {}
    for s in seeds:
        R[s] = run(s)
        print(f"  seed {s}: per-bit accuracy = {R[s]:.2f} (chance 0.50)", flush=True)
    G119 = all(R[s] >= 0.95 for s in seeds)
    print("\n--- VERDICT ---", flush=True)
    print(f"G119 wide-spaced multi-bit (per-bit acc>=0.95 both seeds): {G119}", flush=True)
    if G119:
        print("G119: PASS - wide spacing yields a CLEAN multi-bit content-addressable matter memory (>=0.95/bit) — breakthrough cemented", flush=True)
    else:
        print("G119: NULL/PARTIAL - wide spacing did not reach 0.95/bit (see accuracy)", flush=True)
    out = __import__('pathlib').Path.home() / ".eqmod" / "bet" / "G119"
