"""BP-E209 reverse split-port soft-kill L1; reverse pid1 survives. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((5921,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E209 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 9701 + ti * 59)
            w = World(cfg(seed))
            train_both(w, rng)
            w.active_pattern_id = 2
            clear_state(w)
    ...
    result = {"id": "BP-E209", "bars": {
        "B1_pre_rev_pid2": {"value": a1, "threshold": 0.90, "pass": p1},
        "B2_post_soft_rev_fail": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_rev_pid1_survives": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E209"
