"""BP-C25 free dual talent with local ILW pair scaffold vs free-only control. Headless."""
...
N_SIDE, N_SCAF, T_FULL = 400, 20, 1200
SEEDS, TRIALS = (3801, 3811, 3821), 3
MID = 40.0
LOW, HIGH = (100.0, 2000.0), (500.0, 10000.0)
# local anchors within each half (not crossing midplane)
LA1=np.array([18.,20.,25.]); LA2=np.array([22.,30.,25.])
RA1=np.array([58.,20.,25.]); RA2=np.array([62.,30.,25.])
...
def pair_scaffold(w, rng):
    for _ in range(N_SCAF):
        apply_ilw_pair_write(w, LA1, LA2, 400., 800., rng)
        apply_ilw_pair_write(w, RA1, RA2, 4000., 8000., rng)
...
def run_one(seed, ti, scaffold, t_total):
    w=World(cfg(seed))
    rng=np.random.default_rng(seed*1701+ti*111+int(scaffold)*23)
    if scaffold:
        pair_scaffold(w, rng)
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((3801,),1,300) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C25 start smoke={args.smoke}")
    on,off=[],[]
    for s in seeds:
    ...
    result={"id":"BP-C25","bars":{
        "B1_scaffold_spec":{"value":b1,"threshold":0.90,"pass":p1},
        "B2_ctrl_spec":{"value":b2,"threshold":0.80,"pass":p2},
        "B3_scaffold_pop":{"value":b3,"threshold":0.80,"pass":p3},
        "B4_delta":{"value":b4,"threshold":0.15,"pass":p4},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-C25"; out.mkdir(parents=True,exist_ok=True)
