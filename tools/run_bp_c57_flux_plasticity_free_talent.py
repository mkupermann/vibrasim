"""BP-C57 free dual flux_plasticity_rate vs off. Headless."""
...
def cfg(seed, flux):
    return WorldConfig(
        ...
        flux_plasticity_rate=0.1 if flux else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 9601 + ti * 91 + int(treat) * 181)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5901,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C57 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C57"
