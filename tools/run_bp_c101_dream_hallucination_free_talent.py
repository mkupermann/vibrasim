"""BP-C101 free dual dream_hallucination_strength elevated vs default (dream ON). Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        dream_hallucination_strength=2.0 if elev else 1.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 24201 + ti * 389 + int(treat) * 487)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7821,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C101 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C101"
