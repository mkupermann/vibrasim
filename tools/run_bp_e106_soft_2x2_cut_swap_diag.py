"""BP-E106 soft 2x2 full restore then soft-cut swap diagonal 01+10. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3021,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E106 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*129199+ti*709)
    ...
    result={"id":"BP-E106","bars":{
        "B1_concurrent":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_L0_id":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_L1_id":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E106"; out.mkdir(parents=True,exist_ok=True)
