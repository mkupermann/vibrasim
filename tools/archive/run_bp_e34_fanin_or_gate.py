"""BP-E34 fan-in OR gate via shared mid. Headless."""
...
def peak_R(w, fire_pt=None):
    thr=float(w.config.theta_fire); dt=float(w.config.dt)
    w.k_charge[:]=0; w.k_latch[:]=0
    peak=0.
    for t in range(T_PROP):
        if fire_pt is not None and t%8==0:
            for i in range(w.k_count):
                if w.k_alive[i] and int(w.k_level[i])>=4 and float(np.linalg.norm(w.k_pos[i]-fire_pt))<=8:
                    w.k_charge[i]=thr+5.
        tick(w,dt)
        for i in range(w.k_count):
...
def main(argv=None):
    ...
    seeds,trials=((1061,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E34 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*53023+ti*229)
            w=World(cfg(seed)); train(w,rng)
            b1s.append(peak_R(w,L1)>=1.0)
            w2=World(cfg(seed)); train(w2,rng)
            b2s.append(peak_R(w2,L2)>=1.0)
            w3=World(cfg(seed)); train(w3,rng)
            b3s.append(peak_R(w3,None)<=0.25)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.90,a2>=0.90,a3>=0.90
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E34","bars":{
        "B1_L1":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_L2":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_none":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E34"; out.mkdir(parents=True,exist_ok=True)
