"""BP-E203 bidirectional reverse fire-select R→L. Headless."""
...
def cfg(seed, bidir):
    return WorldConfig(
        ...
        bidirectional_bridges=bool(bidir),
    )
...
def main(argv=None):
    ...
    seeds, trials = ((5681,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E203 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 8501 + ti * 47)
            ...
            b2s.append(reverse_c1(w))
            w2 = World(cfg(seed, False))
            rng2 = np.random.default_rng(seed * 8501 + ti * 47 + 13)
            train_both(w2, rng2)
            clear_state(w2)
            fire_side_band(w2, False, F_HI)
            b3s.append(not reverse_c0(w2))
    a1, a2, a3 = map(float, (np.mean(b1s), np.mean(b2s), np.mean(b3s)))
    p1, p2, p3 = a1 >= 0.70, a2 >= 0.70, a3 >= 0.70
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E203", "bars": {
        "B1_treat_rev_c0": {"value": a1, "threshold": 0.70, "pass": p1},
        "B2_treat_rev_c1": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_ctrl_rev_fail": {"value": a3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E203"
