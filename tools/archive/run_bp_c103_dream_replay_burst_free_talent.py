"""BP-C103 free dual dream_replay_burst_size elevated vs default (dream ON). Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        dream_replay_burst_size=16 if elev else 8,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 25001 + ti * 401 + int(treat) * 499)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7901,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C103 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C103"
