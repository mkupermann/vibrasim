"""BP-E145 cascade AND hard dual wipe, restore L2 first then L1 chain. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3901,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E145 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*168217+ti*997)
    ...
    result={"id":"BP-E145","bars":{
        "B1_dual_silence":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_L2_only_still_off":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_both_on":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E145"; out.mkdir(parents=True,exist_ok=True)
