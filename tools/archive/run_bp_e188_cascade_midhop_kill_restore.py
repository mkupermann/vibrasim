"""BP-E188 cascade mid-hop kill + restore multi-trial. Headless."""
...
SEEDS, TRIALS = (5081, 5091), 8
N_TRAIN, N_WRITE, N_RES, T_PROP = 12, 12, 8, 100
NEAR = 7.0
...
def restore_p0(w, rng):
    for _ in range(4):
        link(w, rng, L0, M0, F0L, F0M, N_RES)
        link(w, rng, M0, R0, F0M, F0R, N_RES)
        idle(w, 3)
...
def main(argv=None):
    ...
    seeds, trials = ((5081,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E188 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 5701 + ti * 23)
            w = World(cfg(seed))
            train_both(w, rng)
            hard_cut_at(w, M0)
            idle(w, 10)
            clear_state(w)
            fire_near(w, L1)
            b1s.append(select_p1(w))
            restore_p0(w, rng)
            clear_state(w)
            fire_near(w, L0)
            b2s.append(select_p0(w))
            clear_state(w)
            fire_near(w, L1)
            b3s.append(select_p1(w))
    a1, a2, a3 = map(float, (np.mean(b1s), np.mean(b2s), np.mean(b3s)))
    p1, p2, p3 = a1 >= 0.80, a2 >= 0.80, a3 >= 0.80
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E188", "bars": {
        "B1_post_kill_p1": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_restore_p0": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_p1_after_restore": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E188"
