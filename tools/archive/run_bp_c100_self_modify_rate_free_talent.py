"""BP-C100 free dual self_modify_rate elevated vs default (self_aware ON). Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        self_modify_rate=0.2 if elev else 0.05,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 23801 + ti * 383 + int(treat) * 479)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7781,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C100 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C100"
