"""BP-C27 free dual talent short pair_decay_time vs default; budget-fit. Headless."""
...
SEEDS, TRIALS = (4321, 4331), 2
MID = 40.0
LOW, HIGH = (100.0, 2000.0), (500.0, 10000.0)
TAU_TREAT, TAU_CTRL = 15.0, 60.0

def cfg(seed, pair_decay):
    return WorldConfig(
        ...
        pair_decay_time=float(pair_decay), triad_decay_time=600.,
...
def run_one(seed, ti, treat, t_total):
    w=World(cfg(seed, TAU_TREAT if treat else TAU_CTRL))
    rng=np.random.default_rng(seed*2001+ti*173+int(treat)*31)
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((4321,),1,200) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C27 start smoke={args.smoke}", flush=True)
    on,off=[],[]
    for s in seeds:
    ...
    out=Path.home()/".eqmod"/"bet"/"BP-C27"; out.mkdir(parents=True,exist_ok=True)
