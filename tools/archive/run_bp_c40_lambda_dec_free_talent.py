"""BP-C40 free dual elevated lambda_dec vs 0. Headless."""
...
def cfg(seed, ldec):
    return WorldConfig(
        ...
        lambda_gen=0., lambda_dec=float(ldec), speed_min=5., speed_max=25.,
...
def run_one(seed, ti, treat, t_total):
    w = World(cfg(seed, 0.01 if treat else 0.0))
    rng = np.random.default_rng(seed * 6201 + ti * 7 + int(treat) * 89)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5181,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C40 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C40"
