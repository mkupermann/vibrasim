"""BP-C31 free dual n_emit>0 vs n_emit=0. Headless."""
...
def cfg(seed, n_emit):
    return WorldConfig(
        ...
        neuron_dynamics_enabled=True, theta_fire=2., t_refractory=0.02, n_emit=int(n_emit),
    )
...
def run_one(seed, ti, treat, t_total):
    w = World(cfg(seed, 4 if treat else 0))
    rng = np.random.default_rng(seed * 3901 + ti * 103 + int(treat) * 47)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((4721,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C31 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C31"
