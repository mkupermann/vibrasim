"""BP-E146 hybrid cascade hard dual wipe selective cascade AND restore. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3921,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E146 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*169217+ti*1009)
    ...
    result={"id":"BP-E146","bars":{
        "B1_both_silent":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_cascade_and_restored":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_or_still_off":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E146"; out.mkdir(parents=True,exist_ok=True)
