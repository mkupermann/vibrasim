"""BP-C105 free dual elevated btsp_potentiation vs default (both btsp ON). Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        btsp_enabled=True,
        btsp_potentiation=150.0 if elev else 50.0,
    )
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7941,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C105 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C105"
