"""BP-E195 pattern-id gated multi-trial switch 1→2→1. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((5401,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E195 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 7101 + ti * 23)
            w = World(cfg(seed))
            train_both(w, rng)
            tag_patterns(w)
    ...
    result = {"id": "BP-E195", "bars": {
        "B1_first_pid1": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_pid2": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_second_pid1": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E195"
