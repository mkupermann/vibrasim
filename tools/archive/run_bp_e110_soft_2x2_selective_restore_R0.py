"""BP-E110 soft 2x2 full restore, dual-cut R0 in-edges, selective restore 00. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3121,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E110 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*133217+ti*743)
            w=World(cfg(seed)); train_all(w,rng)
            dual_cut_all(w)
            for k in ARMS:
                restore(w,rng,k)
            soft_cut(w,"00"); soft_cut(w,"10")
            # B1: R0 silent for both L; R1 still ON
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0])
            ok_l0 = end_near(w,R0)<=0.25 and end_near(w,R1)>=1.0
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L1])
            ok_l1 = end_near(w,R0)<=0.25 and end_near(w,R1)>=1.0
            b1s.append(ok_l0 and ok_l1)
            # selective restore 00 only (disarm already after soft_cut)
            restore(w,rng,"00")
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0])
    ...
    result={"id":"BP-E110","bars":{
        "B1_R0_silent":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_L0_fanout":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_L1_R0_still_off":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E110"; out.mkdir(parents=True,exist_ok=True)
