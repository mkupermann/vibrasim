"""BP-E187 cascade mid-hop M0 hard kill; path1 survives. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((5061,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E187 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 5601 + ti * 29)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_near(w, L0)
    ...
    result = {"id": "BP-E187", "bars": {
        "B1_pre_p0": {"value": a1, "threshold": 0.90, "pass": p1},
        "B2_post_mid_kill_p0_fail": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_p1_survives": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E187"
