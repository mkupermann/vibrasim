"""BP-E119 soft 2x2 split R0a/R0b (non-shared) selective restore 00. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3301,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E119 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*142217+ti*821)
    ...
    result={"id":"BP-E119","bars":{
        "B1_split_silence":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_L0_R0a_fanout":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_L1_R0b_still_off":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E119"; out.mkdir(parents=True,exist_ok=True)
