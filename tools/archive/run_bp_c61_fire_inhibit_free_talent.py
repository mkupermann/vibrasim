"""BP-C61 free dual fire_inhibit_radius vs off. Headless."""
...
def cfg(seed, inhib):
    return WorldConfig(
        ...
        fire_inhibit_radius=8.0 if inhib else 0.0,
        fire_inhibit_frac=0.5,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 10401 + ti * 107 + int(treat) * 199)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6061,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C61 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C61"
