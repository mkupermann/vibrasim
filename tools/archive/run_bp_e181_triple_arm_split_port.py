"""BP-E181 triple-arm split-port fire-select capacity. Headless."""
...
SEEDS, TRIALS = (4821, 4831), 8
N_TRAIN, N_WRITE, T_PROP = 10, 10, 80
MID = 40.0
NEAR = 7.0
# (L, R, fL, fR)
ARMS = [
...
def main(argv=None):
    ...
    seeds, trials = ((4821,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E181 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 4401 + ti * 79)
            w = World(cfg(seed))
            train_all(w, rng)
            oks = []
            for idx in range(3):
                clear_state(w)
                fire_near(w, ARMS[idx][0])
                oks.append(select_arm(w, idx))
            b1s.append(oks[0]); b2s.append(oks[1]); b3s.append(oks[2])
    a1, a2, a3 = map(float, (np.mean(b1s), np.mean(b2s), np.mean(b3s)))
    p1, p2, p3 = a1 >= 0.80, a2 >= 0.80, a3 >= 0.80
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E181", "bars": {
        "B1_c0_select": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_c1_select": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_c2_select": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E181"
