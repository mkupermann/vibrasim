"""BP-C81 free dual graceful_capacity under tight n_nodes_max vs ample. Headless."""
...
def cfg(seed, treat):
    return WorldConfig(
        n_initial_vibrations=0, box_size=(80.,50.,50.), n_vibrations_max=8192,
        n_nodes_max=TREAT_NODES if treat else CTRL_NODES,
        ...
        graceful_capacity=True if treat else False,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 14201 + ti * 251 + int(treat) * 353)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6821,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C81 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C81"
