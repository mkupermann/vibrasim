"""BP-C79 free dual compartment_boundary vs off. Headless."""
...
def cfg(seed, bound):
    return WorldConfig(
        ...
        compartment_boundary=1.0 if bound else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 13801 + ti * 229 + int(treat) * 331)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6781,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C79 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C79"
