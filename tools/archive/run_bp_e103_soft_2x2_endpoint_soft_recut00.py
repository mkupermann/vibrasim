"""BP-E103 soft 2x2 full restore then soft re-cut at R0 endpoint. Headless."""
...
fL = 400.
I_R0 = R0.copy()  # endpoint soft re-cut
...
def train_all(w,rng):
    for a in ARMS.values():
        ...
        idle(w,3)
    # also plant weaken-capable nodes at R0 for endpoint cut
    for _ in range(N_WRITE):
        apply_ilw_port_event(w,I_R0,rng,seed_freq=9500.)
    idle(w,3)
...
def soft_cut_at(w, pos):
    set_weaken_at(w,pos)
    fire_phase(w,[pos], n=28)
    disarm(w)

def dual_cut_all(w):
    for k in ARMS:
        soft_cut_at(w,ARMS[k]["I"])

def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument("--smoke",action="store_true")
    args=p.parse_args(argv)
    seeds,trials=((2921,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E103 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*126191+ti*683)
    ...
    result={"id":"BP-E103","bars":{
        "B1_L0_fanout":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_endpoint_cut":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_L1_both":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E103"; out.mkdir(parents=True,exist_ok=True)
