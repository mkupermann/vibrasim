"""BP-C16 free dual talent with ilw_strength_decay vs control. Headless."""
...
def run_one(seed, ti, decay, t_total):
    ...
    rng=np.random.default_rng(seed*2401+ti*73+int(decay)*23)
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((2271,),1,300) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C16 start smoke={args.smoke}")
    on,off=[],[]
    for s in seeds:
    ...
    out=Path.home()/".eqmod"/"bet"/"BP-C16"; out.mkdir(parents=True,exist_ok=True)
