"""BP-C12 free dual talent with STDP vs no-STDP control. Headless."""
...
def run_one(seed, ti, stdp, t_total):
    w=World(cfg(seed, stdp))
    rng=np.random.default_rng(seed*2003+ti*59+int(stdp)*11)
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((1831,),1,300) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C12 start smoke={args.smoke}")
    on,off=[],[]
    for s in seeds:
    ...
    result={"id":"BP-C12","bars":{
        "B1_stdp_spec":{"value":b1,"threshold":0.90,"pass":p1},
        "B2_ctrl_spec":{"value":b2,"threshold":0.80,"pass":p2},
        "B3_stdp_pop":{"value":b3,"threshold":0.80,"pass":p3},
        "B4_delta":{"value":b4,"threshold":0.15,"pass":p4},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-C12"; out.mkdir(parents=True,exist_ok=True)
