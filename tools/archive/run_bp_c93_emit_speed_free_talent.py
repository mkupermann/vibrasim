"""BP-C93 free dual emit_speed elevated vs default under n_emit=2. Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        emit_speed=60.0 if elev else 30.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 21001 + ti * 331 + int(treat) * 431)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7501,), 1, 150) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C93 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C93"
