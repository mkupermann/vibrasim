"""BP-C47 free dual global_wta_k vs off. Headless."""
...
def run_one(seed, ti, treat, t_total):
    w = World(cfg(seed, 8 if treat else 0))
    rng = np.random.default_rng(seed * 7501 + ti * 41 + int(treat) * 127)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5481,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C47 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C47"
