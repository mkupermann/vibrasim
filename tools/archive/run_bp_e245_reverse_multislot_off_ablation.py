"""BP-E245 cascade reverse multislot ON control vs OFF ablation. Headless."""
...
def run_arm(seed, ti, multislot):
    rng = np.random.default_rng(seed * 19601 + ti * 227 + int(multislot) * 13)
...
def main(argv=None):
    ...
    seeds, trials = ((7361,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E245 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
    ...
    result = {"id": "BP-E245", "bars": {
        "B1_multislot_on_both": {"value": b1, "threshold": 0.80, "pass": p1},
        "B2_multislot_off_p0": {"value": b2, "threshold": 0.80, "pass": p2},
        "B3_multislot_off_p1": {"value": b3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E245"
