"""BP-C80 free dual emit_band_ratios decade-skew vs default (n_emit=8 both). Headless."""
...
SEEDS, TRIALS = (6801, 6811), 2
MID = 40.0
LOW, HIGH = (100.0, 2000.0), (500.0, 10000.0)
TREAT_RATIOS = (0.01, 1.0, 100.0)
CTRL_RATIOS = (0.08, 1.0, 12.5)
...
def cfg(seed, treat):
    return WorldConfig(
        ...
        neuron_dynamics_enabled=True, theta_fire=2., t_refractory=0.02, n_emit=8,
        emit_band_ratios=TREAT_RATIOS if treat else CTRL_RATIOS,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 14001 + ti * 241 + int(treat) * 347)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6801,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C80 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C80"
