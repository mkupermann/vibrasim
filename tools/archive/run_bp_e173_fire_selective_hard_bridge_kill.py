"""BP-E173 fire selective residual hard R bridge kill then restore. Headless."""
...
SEEDS, TRIALS = (4601, 4611), 8
N_TRAIN, N_WRITE, N_RES, T_PROP = 12, 12, 8, 80
MID = 40.0
...
def train_both(w, rng, n_train=N_TRAIN, n_write=N_WRITE):
...
def main(argv=None):
    ...
    seeds, trials = ((4601,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E173 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 3301 + ti * 127)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_L_band(w, F_LO)
            b1s.append(select_lo(w))
            hard_cut_port(w, PORT_R)
            idle(w, 10)
            clear_state(w)
            fire_L_band(w, F_LO)
            b2s.append(not select_lo(w))
            train_both(w, rng, n_train=4, n_write=N_RES)
            clear_state(w)
            fire_L_band(w, F_LO)
            b3s.append(select_lo(w))
    a1, a2, a3 = map(float, (np.mean(b1s), np.mean(b2s), np.mean(b3s)))
    p1, p2, p3 = a1 >= 0.90, a2 >= 0.70, a3 >= 0.80
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E173", "bars": {
        "B1_pre_kill_select": {"value": a1, "threshold": 0.90, "pass": p1},
        "B2_post_kill_fail": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_restore_select": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E173"
