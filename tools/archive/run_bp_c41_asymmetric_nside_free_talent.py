"""BP-C41 free dual asymmetric N_SIDE vs equal. Headless."""
...
def run_one(seed, ti, treat, t_total):
    w = World(cfg(seed))
    rng = np.random.default_rng(seed * 6401 + ti * 3 + int(treat) * 97)
    if treat:
        inject(w, rng, 500, 8, 32, LOW[0], LOW[1])
        inject(w, rng, 150, 48, 72, HIGH[0], HIGH[1])
    else:
        inject(w, rng, 325, 8, 32, LOW[0], LOW[1])
        inject(w, rng, 325, 48, 72, HIGH[0], HIGH[1])
    evolve(w, t_total)
    return sides(w)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5221,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C41 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C41"
