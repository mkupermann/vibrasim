"""BP-C14 free dual talent with bridge_leak vs no-leak control. Headless."""
...
def cfg(seed, leak):
    return WorldConfig(
        ...
        bridge_leak_rate=float(LEAK if leak else 0.0),
    )
...
def run_one(seed, ti, leak, t_total):
    w=World(cfg(seed, leak))
    rng=np.random.default_rng(seed*2203+ti*67+int(leak)*17)
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((2011,),1,300) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C14 start smoke={args.smoke}")
    on,off=[],[]
    for s in seeds:
    ...
    result={"id":"BP-C14","bars":{
        "B1_leak_spec":{"value":b1,"threshold":0.90,"pass":p1},
        "B2_ctrl_spec":{"value":b2,"threshold":0.80,"pass":p2},
        "B3_leak_pop":{"value":b3,"threshold":0.80,"pass":p3},
        "B4_delta":{"value":b4,"threshold":0.15,"pass":p4},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-C14"; out.mkdir(parents=True,exist_ok=True)
