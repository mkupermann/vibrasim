"""BP-E36 four-hop L-A-B-C-R relay. Headless."""
...
SEEDS, TRIALS = (1101, 1111), 8
N_WRITE, T_PROP = 10, 100
P = [np.array([10.+i*14., 25., 25.]) for i in range(5)]
F = [400., 1000., 2000., 4000., 8000.]
...
def train(w,rng, skip_mid=False):
    for i in range(4):
        if skip_mid and i==2: continue
        for _ in range(N_WRITE):
            apply_ilw_pair_write(w,P[i],P[i+1],F[i],F[i+1],rng)
        idle(w,6)
    ...
    for t in range(T_PROP):
        if t%5==0:
            for i in range(w.k_count):
                if w.k_alive[i] and int(w.k_level[i])>=4 and float(w.k_pos[i,0])<15:
                    w.k_charge[i]=thr+5.
        tick(w,dt)
        for i in range(w.k_count):
...
def main(argv=None):
    ...
    seeds,trials=((1101,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E36 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*55031+ti*239)
            w=World(cfg(seed)); train(w,rng,False)
            b3s.append(n_br(w)>=4); b1s.append(peak_end(w)>=1.0)
            w0=World(cfg(seed)); train(w0,rng,True)
            b2s.append(peak_end(w0)<=0.25)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.85,a2>=0.90,a3>=0.90
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E36","bars":{
        "B1_fourhop":{"value":a1,"threshold":0.85,"pass":p1},
        "B2_broken":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_bridges":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E36"; out.mkdir(parents=True,exist_ok=True)
