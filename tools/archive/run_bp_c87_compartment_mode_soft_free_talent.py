"""BP-C87 free dual compartment_mode soft vs clamp (compartment_k on both). Headless."""
...
def cfg(seed, soft):
    return WorldConfig(
        ...
        compartment_k=1.0,
        compartment_centre=(40.0, 25.0, 25.0),
        compartment_radius=35.0,
        compartment_mode="soft" if soft else "clamp",
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 16601 + ti * 283 + int(treat) * 389)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7061,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C87 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C87"
