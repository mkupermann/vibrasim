"""BP-C22 free dual talent large box vs standard box control. Headless."""
...
def inject(w, rng, n, x0, x1, f0, f1, y0, y1):
    ...
    for k,i in enumerate(slots):
        i=int(i)
        w.s_pos[i]=[rng.uniform(x0,x1), rng.uniform(y0,y1), rng.uniform(8, min(42, w.config.box_size[2]-8))]
...
def run_one(seed, ti, large, t_total):
    c, mid = cfg(seed, large)
    w=World(c)
    rng=np.random.default_rng(seed*1403+ti*107+int(large)*13)
    if large:
        inject(w,rng,N_SIDE,10,50,LOW[0],LOW[1],10,60)
        inject(w,rng,N_SIDE,70,110,HIGH[0],HIGH[1],10,60)
    else:
        inject(w,rng,N_SIDE,8,32,LOW[0],LOW[1],8,42)
        inject(w,rng,N_SIDE,48,72,HIGH[0],HIGH[1],8,42)
    evolve(w,t_total)
    return sides(w, mid)
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((2961,),1,300) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C22 start smoke={args.smoke}")
    on,off=[],[]
    for s in seeds:
    ...
    result={"id":"BP-C22","bars":{
        "B1_large_spec":{"value":b1,"threshold":0.90,"pass":p1},
        "B2_ctrl_spec":{"value":b2,"threshold":0.80,"pass":p2},
        "B3_large_pop":{"value":b3,"threshold":0.80,"pass":p3},
        "B4_delta":{"value":b4,"threshold":0.15,"pass":p4},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-C22"; out.mkdir(parents=True,exist_ok=True)
