"""BP-C51 free dual mol_fusion_enabled vs off. Headless."""
...
def cfg(seed, fusion):
    return WorldConfig(
        ...
        mol_fusion_enabled=bool(fusion),
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 8401 + ti * 67 + int(treat) * 151)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5661,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C51 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C51"
