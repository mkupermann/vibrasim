"""BP-C76 free dual fire_zero_latch_radius vs off. Headless."""
...
def cfg(seed, zero):
    return WorldConfig(
        ...
        fire_zero_latch_radius=8.0 if zero else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 13201 + ti * 199 + int(treat) * 311)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6661,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C76 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C76"
