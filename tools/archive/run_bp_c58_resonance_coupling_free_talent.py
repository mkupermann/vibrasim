"""BP-C58 free dual resonance_coupling vs off. Headless."""
...
def cfg(seed, res):
    return WorldConfig(
        ...
        resonance_coupling=0.5 if res else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 9801 + ti * 97 + int(treat) * 191)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5941,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C58 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C58"
