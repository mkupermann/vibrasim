"""BP-C102 free dual video_emit_pair_band elevated vs off. Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        video_emit_pair_band=0.15 if elev else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 24601 + ti * 397 + int(treat) * 491)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7861,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C102 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C102"
