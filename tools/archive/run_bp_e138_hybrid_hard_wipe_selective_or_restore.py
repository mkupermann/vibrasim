"""BP-E138 hybrid hard dual wipe I1+I3 then selective OR-only restore. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3721,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E138 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*161217+ti*947)
    ...
    result={"id":"BP-E138","bars":{
        "B1_both_silent":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_or_restored":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_and_still_off":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E138"; out.mkdir(parents=True,exist_ok=True)
