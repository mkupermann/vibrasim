"""BP-E215 cascade reverse under G12 pattern gate. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((6161,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E215 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 10901 + ti * 73)
            w = World(cfg(seed))
    ...
    result = {"id": "BP-E215", "bars": {
        "B1_correct_rev_p0": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_wrong_rev_fail": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_correct_rev_p1": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E215"
