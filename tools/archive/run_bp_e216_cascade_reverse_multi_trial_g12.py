"""BP-E216 cascade reverse multi-trial switch under G12. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((6201,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E216 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 11101 + ti * 79)
            w = World(cfg(seed))
    ...
    result = {"id": "BP-E216", "bars": {
        "B1_rev_p0": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_rev_p1": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_rev_p0_again": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E216"
