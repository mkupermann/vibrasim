"""BP-E228 cascade reverse hard mid-kill M1 then retrain restore. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((6761,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E228 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 13701 + ti * 127)
            w = World(cfg(seed))
            train_both(w, rng)
            hard_cut_at(w, M1)
            idle(w, 10)
    ...
    result = {"id": "BP-E228", "bars": {
        "B1_post_hard_rev_fail": {"value": a1, "threshold": 0.70, "pass": p1},
        "B2_rev_p0_survives": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_restore_rev_p1": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E228"
