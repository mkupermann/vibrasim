"""BP-C17 C16 strength-decay free talent replication on held-out seeds. Headless."""
...
def run_one(seed, ti, decay, t_total):
    ...
    rng=np.random.default_rng(seed*2503+ti*79+int(decay)*29)
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((2301,),1,300) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C17 start smoke={args.smoke}")
    on,off=[],[]
    for s in seeds:
    ...
    result={"id":"BP-C17","bars":{
        "B1_decay_spec":{"value":b1,"threshold":0.90,"pass":p1},
    ...
    },"verdict":verdict,"seeds":list(seeds),"tau":TAU}
    out=Path.home()/".eqmod"/"bet"/"BP-C17"; out.mkdir(parents=True,exist_ok=True)
