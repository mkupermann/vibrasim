"""BP-C35 free dual lambda_gen ambient regen vs 0. Headless."""
...
def cfg(seed, lgen):
    return WorldConfig(
        ...
        lambda_gen=float(lgen), lambda_dec=0., speed_min=5., speed_max=25.,
...
def run_one(seed, ti, treat, t_total):
    w = World(cfg(seed, 0.001 if treat else 0.0))
    rng = np.random.default_rng(seed * 4901 + ti * 59 + int(treat) * 67)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((4921,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C35 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C35"
