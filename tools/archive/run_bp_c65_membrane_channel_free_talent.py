"""BP-C65 free dual membrane_channel_k vs off. Headless."""
...
def cfg(seed, mem):
    return WorldConfig(
        ...
        membrane_channel_k=1.0 if mem else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 11201 + ti * 131 + int(treat) * 233)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6221,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C65 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C65"
