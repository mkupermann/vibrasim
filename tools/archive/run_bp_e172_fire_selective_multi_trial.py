"""BP-E172 multi-trial fire selective residual same world A→B→A. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((4581,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E172 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 3201 + ti * 131)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_L_band(w, F_LO)
    ...
    result = {"id": "BP-E172", "bars": {
        "B1_first_Llo": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_Lhi": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_second_Llo": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E172"
