"""BP-E214 cascade reverse fire-select R→L multi-hop. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((6121,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E214 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 10701 + ti * 71)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_near(w, R0)
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E214"
