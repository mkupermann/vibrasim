"""BP-C53 free dual bridge_lock_threshold vs off. Headless."""
...
def cfg(seed, lock):
    return WorldConfig(
        ...
        stdp_enabled=True,
        bridge_lock_threshold=0.5 if lock else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 8801 + ti * 73 + int(treat) * 163)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5741,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C53 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C53"
