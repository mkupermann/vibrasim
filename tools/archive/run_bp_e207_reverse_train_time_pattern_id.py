"""BP-E207 reverse fire-select train-time pattern_id no post-hoc. Headless."""
...
def fire_R_band(w, f_target, n=T_PROP):
...
def main(argv=None):
    ...
    seeds, trials = ((5841,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E207 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 9301 + ti * 55)
            w = World(cfg(seed))
    ...
    result = {"id": "BP-E207", "bars": {
        "B1_correct_rev_c0": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_wrong_rev_fail": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_correct_rev_c1": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E207"
