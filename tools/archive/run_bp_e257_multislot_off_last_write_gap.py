"""BP-E257 multislot OFF last-write residual after temporal gap. Headless."""
...
def main(argv=None):
    ...
    seeds, trials, t_gap = ((7761,), 2, 40) if args.smoke else (SEEDS, TRIALS, T_GAP)
    print(f"BP-E257 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 23601 + ti * 307)
            w = World(cfg(seed))
            ...
            b1s.append(not selective_A(predA))
            LiB = bridged_L_near(w, PB[0])
            predB = peak_partner_R(w, LiB)
            b2s.append(selective_B(predB))
            w2 = World(cfg(seed + 5))
            rng2 = np.random.default_rng(seed * 23601 + ti * 307 + 9)
            train_pair(w2, rng2, PA[0], PA[1])
            LiA2 = bridged_L_near(w2, PA[0])
            predA2 = peak_partner_R(w2, LiA2)
            b3s.append(selective_A(predA2))
    b1 = float(np.mean(b1s)); b2 = float(np.mean(b2s)); b3 = float(np.mean(b3s))
    p1, p2, p3 = b1 >= 0.70, b2 >= 0.80, b3 >= 0.80
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E257", "bars": {
        "B1_A_residual_fails": {"value": b1, "threshold": 0.70, "pass": p1},
        "B2_B_selective": {"value": b2, "threshold": 0.80, "pass": p2},
        "B3_A_only_ok": {"value": b3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E257"
