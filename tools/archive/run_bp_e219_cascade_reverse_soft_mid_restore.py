"""BP-E219 cascade reverse soft mid-kill then retrain restore. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((6321,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E219 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 11701 + ti * 91)
            w = World(cfg(seed))
            train_both(w, rng)
            soft_cut_at(w, M0)
            idle(w, 10)
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E219"
