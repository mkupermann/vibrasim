"""BP-E200 split-port G12 soft-kill R1 then retrain restore pid2. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((5561,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E200 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 7901 + ti * 53)
            w = World(cfg(seed))
            train_both(w, rng)
            soft_cut_at(w, R1)
            idle(w, 10)
            ...
            clear_state(w)
            fire_near(w, L1)
            b1s.append(not select_c1(w))
            w.active_pattern_id = 1
            clear_state(w)
            fire_near(w, L0)
            b2s.append(select_c0(w))
    ...
    result = {"id": "BP-E200", "bars": {
        "B1_post_soft_pid2_fail": {"value": a1, "threshold": 0.70, "pass": p1},
        "B2_pid1_survives": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_restore_pid2": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E200"
