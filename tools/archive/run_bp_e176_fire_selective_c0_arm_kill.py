"""BP-E176 fire-select c0-arm hard kill; c1 select survives. Headless."""
...
def band_match(f, f_target):
    return abs(math.log(max(f, 1.0)) - math.log(max(f_target, 1.0))) <= LOG_TOL

def hard_kill_c0_arm(w):
    """Kill bridges from L-lo and R-hi endpoints only (c0 association)."""
    ...
    for i in range(w.k_count):
        ...
        x = float(w.k_pos[i, 0])
        # L-lo endpoint
        if x < MID and band_match(f, F_LO):
            w.k_kill_bridge_emitter[i] = 1
        # R-hi endpoint
        if x >= MID and f >= F_MID:
            w.k_kill_bridge_emitter[i] = 1
    thr = float(w.config.theta_fire)
    dt = float(w.config.dt)
    for t in range(36):
        if t % 5 == 0:
            for i in range(w.k_count):
                if not w.k_alive[i] or int(w.k_level[i]) < 4:
                    continue
                if int(w.k_kill_bridge_emitter[i]) != 1:
                    continue
                w.k_charge[i] = thr + 5.
        tick(w, dt); w.t += dt
    if hasattr(w, "k_kill_bridge_emitter"):
...
def main(argv=None):
    ...
    seeds, trials = ((4661,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E176 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 3601 + ti * 113)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_L_band(w, F_LO)
    ...
    result = {"id": "BP-E176", "bars": {
        "B1_pre_Llo_select": {"value": a1, "threshold": 0.90, "pass": p1},
        "B2_post_c0_kill_Llo_fail": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_post_c0_kill_Lhi_select": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E176"
