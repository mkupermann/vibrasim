"""BP-C90 free dual bridge_charge_prop_rate on vs off. Headless."""
...
def cfg(seed, prop):
    return WorldConfig(
        ...
        bridge_charge_prop_rate=2.5 if prop else 0.0,
        bridge_prop_min_strength=0.0,
        charge_latch_enabled=True, charge_latch_tau=0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 17801 + ti * 311 + int(treat) * 409)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7181,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C90 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C90"
