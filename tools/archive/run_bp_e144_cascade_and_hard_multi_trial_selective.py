"""BP-E144 cascade AND multi-trial hard dual wipe selective L1 re-arm. Headless."""
...
def restore_L1_chain(w,rng):
    link(w,rng,L1,M,400.,1500.,N_RES)
    link(w,rng,M,A,1500.,3000.,N_RES)
    link(w,rng,A,R,3000.,6000.,N_RES)
    arm_gate(w)

def restore_L2(w,rng):
...
def main(argv=None):
    ...
    seeds,trials=((3881,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E144 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*167217+ti*991)
            w=World(cfg(seed)); train(w,rng)
            dual_wipe(w)
            restore_L1_chain(w,rng)
            b1s.append(peak_R(w,[L1,L2])<=0.25)
            restore_L2(w,rng)
            b2s.append(peak_R(w,[L1,L2])>=1.0)
            dual_wipe(w)
            restore_L1_chain(w,rng)
    ...
    result={"id":"BP-E144","bars":{
        "B1_first_L1_only_off":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_first_both_on":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_second_L1_only_off":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E144"; out.mkdir(parents=True,exist_ok=True)
