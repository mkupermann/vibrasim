"""BP-E247 cascade reverse charge_latch ON control vs OFF ablation. Headless."""
...
def cfg(seed, latch):
    return WorldConfig(
        ...
        charge_latch_enabled=bool(latch), charge_latch_tau=0.,
    )
...
def peak_near(w, pos):
    ...
    for i in range(w.k_count):
        if w.k_alive[i] and int(w.k_level[i]) >= 4 and float(np.linalg.norm(w.k_pos[i] - pos)) <= NEAR:
            if hasattr(w, "k_latch") and bool(w.config.charge_latch_enabled):
                lat = float(w.k_latch[i])
            else:
                lat = float(w.k_charge[i])
            m = max(m, lat)
    return m
...
def run_arm(seed, ti, latch):
    rng = np.random.default_rng(seed * 20001 + ti * 233 + int(latch) * 17)
    w = World(cfg(seed, latch))
    train_both(w, rng)
    clear_state(w)
    fire_near(w, R0)
    ok0 = reverse_p0(w)
...
def main(argv=None):
    ...
    seeds, trials = ((7401,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E247 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            ok0_on, ok1_on = run_arm(seed, ti, True)
            b1s.append(ok0_on and ok1_on)
            ok0_off, ok1_off = run_arm(seed, ti, False)
            b2s.append(ok0_off)
            b3s.append(ok1_off)
    b1 = float(np.mean(b1s)); b2 = float(np.mean(b2s)); b3 = float(np.mean(b3s))
    p1, p2, p3 = b1 >= 0.80, b2 >= 0.80, b3 >= 0.80
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E247", "bars": {
        "B1_latch_on_both": {"value": b1, "threshold": 0.80, "pass": p1},
        "B2_latch_off_p0": {"value": b2, "threshold": 0.80, "pass": p2},
        "B3_latch_off_p1": {"value": b3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E247"
