"""BP-E56 dual 3-hop paths + selective hard kill path0. Headless."""
...
SEEDS, TRIALS = (1641, 1651), 6
N_WRITE, T_PROP, N_RES = 10, 50, 8
# path0 y=18, path1 y=32 — separation 14 > kill r=12
...
def train(w,rng):
    ...
    for _ in range(N_WRITE):
        apply_ilw_port_event(w,I0,rng,seed_freq=9000.)
    idle(w,4)
    for i in range(w.k_count):
        if w.k_alive[i] and int(w.k_level[i])>=4 and float(np.linalg.norm(w.k_pos[i]-I0))<=6:
...
def main(argv=None):
    ...
    seeds,trials=((1641,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E56 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*79023+ti*383)
