"""BP-E50 soft-cut mid hop of L-A-B-R, restore A-B only. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((1521,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E50 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*73099+ti*349)
    ...
    result={"id":"BP-E50","bars":{
        "B1_on":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_mid_off":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_mid_restore":{"value":a3,"threshold":0.85,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E50"; out.mkdir(parents=True,exist_ok=True)
