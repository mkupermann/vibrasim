"""BP-C66 free dual anchor_damping vs off. Headless."""
...
def cfg(seed, anchor):
    return WorldConfig(
        ...
        anchor_damping=0.7 if anchor else 0.0,
        anchor_bond_min=2,
        anchor_age=10.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 11401 + ti * 137 + int(treat) * 239)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6261,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C66 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C66"
