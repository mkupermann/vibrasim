"""BP-E212 reverse split-port hard-kill L1; reverse pid1 survives. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((6041,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E212 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 10301 + ti * 65)
            w = World(cfg(seed))
            train_both(w, rng)
            w.active_pattern_id = 2
            clear_state(w)
            fire_near(w, R1)
            b1s.append(reverse_c1(w))
            hard_cut_at(w, L1)
            idle(w, 10)
            clear_state(w)
            fire_near(w, R1)
            b2s.append(not reverse_c1(w))
            w.active_pattern_id = 1
            clear_state(w)
            fire_near(w, R0)
            b3s.append(reverse_c0(w))
    a1, a2, a3 = map(float, (np.mean(b1s), np.mean(b2s), np.mean(b3s)))
    p1, p2, p3 = a1 >= 0.90, a2 >= 0.70, a3 >= 0.80
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E212", "bars": {
        "B1_pre_rev_pid2": {"value": a1, "threshold": 0.90, "pass": p1},
        "B2_post_hard_rev_fail": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_rev_pid1_survives": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E212"
