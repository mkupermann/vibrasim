"""BP-C50 free dual lateral_inhibition + stdp vs stdp-only. Headless."""
...
def cfg(seed, lat):
    return WorldConfig(
        ...
        stdp_enabled=True,
        lateral_inhibition_enabled=bool(lat),
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 8201 + ti * 61 + int(treat) * 149)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5621,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C50 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C50"
