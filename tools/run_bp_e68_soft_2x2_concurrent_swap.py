"""BP-E68 soft 2x2 concurrent dual-drive under swap. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((1991,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E68 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*91063+ti*457)
    ...
    result={"id":"BP-E68","bars":{
        "B1_concurrent_both":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_L0_to_R1":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_L1_to_R0":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E68"; out.mkdir(parents=True,exist_ok=True)
