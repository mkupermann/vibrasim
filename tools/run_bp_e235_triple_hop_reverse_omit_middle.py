"""BP-E235 triple-hop reverse incomplete omit middle M-A hops. Headless."""
...
def train_omit_middle(w, rng):
    # L-M and A-R only; no M-A
...
def main(argv=None):
    ...
    seeds, trials = ((7081,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E235 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 16801 + ti * 167)
            w = World(cfg(seed))
            train_omit_middle(w, rng)
            clear_state(w)
            fire_near(w, R0)
            ...
            w2 = World(cfg(seed + 23))
            rng2 = np.random.default_rng(seed * 16801 + ti * 167 + 11)
            train_full(w2, rng2)
            clear_state(w2)
            fire_near(w2, R0)
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E235"
