"""BP-E96 soft MUX dual-cut full restore then soft re-cut path0. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((2761,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E96 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*119163+ti*643)
    ...
    out=Path.home()/".eqmod"/"bet"/"BP-E96"; out.mkdir(parents=True,exist_ok=True)
