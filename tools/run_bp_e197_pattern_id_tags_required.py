"""BP-E197 pattern-id tags load-bearing for wrong-arm block. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((5441,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E197 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 7301 + ti * 31)
            w = World(cfg(seed))
            train_untagged(w, rng)
            w.active_pattern_id = 1
            clear_state(w)
            fire_L_band(w, F_LO)
            ...
            rng2 = np.random.default_rng(seed * 7301 + ti * 31 + 5)
            w2 = World(cfg(seed + 5))
            train_tagged(w2, rng2)
            w2.active_pattern_id = 1
            clear_state(w2)
            fire_L_band(w2, F_HI)
            b2s.append(not select_hi(w2))
    a1, a2, a3 = map(float, (np.mean(b1s), np.mean(b2s), np.mean(b3s)))
    p1, p2, p3 = a1 >= 0.80, a2 >= 0.70, a3 >= 0.70
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E197", "bars": {
        "B1_notag_correct_select": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_tagged_wrong_arm_fail": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_notag_wrong_arm_selects": {"value": a3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E197"
