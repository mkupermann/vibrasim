"""BP-E167 temporal-gap L then R association residual. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((4461,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E167 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 2601 + ti * 157)
            w = World(cfg(seed))
            ...
            rng2 = np.random.default_rng(seed * 2601 + ti * 157 + 33)
            w2 = World(cfg(seed + 23))
            for _ in range(N_TRAIN):
                write_port(w2, rng2, PORT_L, F_LO)
                idle(w2, T_GAP + 8)
            write_port(w2, rng2, PORT_L, F_LO)
            idle(w2, T_IDLE)
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E167"
