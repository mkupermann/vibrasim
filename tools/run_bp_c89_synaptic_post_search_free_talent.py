"""BP-C89 free dual synaptic_post_search_samples elevated vs default. Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        synaptic_post_search_samples=5 if elev else 1,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 17401 + ti * 307 + int(treat) * 401)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7141,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C89 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C89"
