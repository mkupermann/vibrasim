"""BP-C44 wide freq_tol free dual wall ON vs OFF ablation. Headless."""
...
def cfg(seed, wall):
    return WorldConfig(
        ...
        rng_seed=seed, r_1=5., r_2=28., freq_tolerance=FT,
...
def run_one(seed, ti, wall, t_total):
    ...
    rng = np.random.default_rng(seed * 6801 + ti * 13 + int(wall) * 109)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5331,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C44 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    result = {"id": "BP-C44", "bars": {
        "B1_wall_on_spec": {"value": b1, "threshold": 0.90, "pass": p1},
        "B2_wall_off_spec": {"value": b2, "threshold": 0.80, "pass": p2},
        "B3_wall_on_pop": {"value": b3, "threshold": 0.80, "pass": p3},
        "B4_delta": {"value": b4, "threshold": 0.15, "pass": p4},
    }, "n": len(on), "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-C44"
