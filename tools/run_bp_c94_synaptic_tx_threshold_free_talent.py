"""BP-C94 free dual synaptic_transmission_threshold elevated vs default. Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        synaptic_transmission_threshold=20.0 if elev else 5.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 21401 + ti * 337 + int(treat) * 439)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7541,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C94 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C94"
