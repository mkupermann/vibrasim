"""BP-E13 cross-mid charge propagation via bridges. Headless."""
...
SEEDS, TRIALS = (461, 471), 10
N_WRITE, T_PROP, MID = 12, 120, 40.0
...
def make_cfg(seed: int, valence: int) -> WorldConfig:
    return WorldConfig(
        ...
        theta_fire=2.0,
        t_refractory=0.02,
        n_emit=0,  # no free-vib flood from fires
...
def mean_charge_side(w: World, left: bool) -> float:
    s, n = 0.0, 0
    for i in range(w.k_count):
        if not w.k_alive[i] or int(w.k_level[i]) < 4:
            continue
        is_l = float(w.k_pos[i, 0]) < MID
        if is_l != left:
            continue
...
def main(argv=None) -> int:
    ...
    seeds, trials = ((461,), 3) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E13 start smoke={args.smoke}")
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 29031 + ti * 103)
            # treatment
            w = World(make_cfg(seed, 4))
            write_dual(w, rng)
            idle(w, 80)  # form bridges
            b3s.append(n_cross(w) >= 1)
            cR0 = mean_charge_side(w, left=False)
            force_fire_left(w)
            idle(w, T_PROP)
            dR = mean_charge_side(w, left=False) - cR0
            b1s.append(dR >= 1.0)
            # control no valence
            w0 = World(make_cfg(seed, 0))
            write_dual(w0, rng)
            idle(w0, 80)
            cR00 = mean_charge_side(w0, left=False)
            force_fire_left(w0)
            idle(w0, T_PROP)
            dR0 = mean_charge_side(w0, left=False) - cR00
            b2s.append(dR0 <= 0.25)
    ...
    result = {
        "id": "BP-E13",
        "bars": {
            "B1_dR_treat": {"value": a1, "threshold": 0.85, "pass": p1},
            "B2_dR_nobridge": {"value": a2, "threshold": 0.85, "pass": p2},
            "B3_cross": {"value": a3, "threshold": 0.90, "pass": p3},
        },
        "verdict": verdict,
        "sample_dR_hint": "fractional bars only",
    }
    out = Path.home() / ".eqmod" / "bet" / "BP-E13"
