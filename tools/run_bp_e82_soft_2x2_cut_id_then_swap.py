"""BP-E82 soft 2x2 dual-cut, identity restore, then swap reconfig. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((2481,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E82 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*105109+ti*563)
            w=World(cfg(seed)); train_all(w,rng)
            dual_cut_all(w)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0,L1])
            b1s.append(end_near(w,R0)<=0.25 and end_near(w,R1)<=0.25)
            for k in ("00","11"):
                restore(w,rng,k)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0,L1])
            conc=end_near(w,R0)>=1.0 and end_near(w,R1)>=1.0
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0])
            isol=end_near(w,R0)>=1.0 and end_near(w,R1)<=0.25
            b2s.append(conc and isol)
            # reconfig to swap: cut identity, restore swap
            for k in ("00","11"):
                soft_cut_key(w,k)
            for k in ("01","10"):
                restore(w,rng,k)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0])
            b3s.append(end_near(w,R1)>=1.0 and end_near(w,R0)<=0.25)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.75
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E82","bars":{
        "B1_both_off":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_identity":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_swap":{"value":a3,"threshold":0.75,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E82"; out.mkdir(parents=True,exist_ok=True)
