"""BP-E156 port dual decade hard kill wipe then ILW restore. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((4161,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E156 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 1921 + ti * 139)
            w = World(cfg(seed))
            write_dual(w, rng)
            idle(w, T_IDLE)
            b1s.append(ordered(w))
            hard_cut_port(w, PORT_L)
            hard_cut_port(w, PORT_R)
            idle(w, 40)
            b2s.append(not ordered(w))
    ...
    result = {"id": "BP-E156", "bars": {
        "B1_initial_ordered": {"value": a1, "threshold": 0.90, "pass": p1},
        "B2_post_hard_wipe_disrupted": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_restore_ordered": {"value": a3, "threshold": 0.90, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E156"
