"""BP-E91 hard 3-path MUX dual-kill then full restore all paths. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((2661,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E91 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*114149+ti*613)
    ...
    out=Path.home()/".eqmod"/"bet"/"BP-E91"; out.mkdir(parents=True,exist_ok=True)
