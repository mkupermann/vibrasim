"""BP-E95 hard DEMUX dual-kill full restore all arms + hard re-cut0. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((2741,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E95 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*118159+ti*641)
    ...
    result={"id":"BP-E95","bars":{
        "B1_all_off":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_all_on":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_hard_cut0":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E95"; out.mkdir(parents=True,exist_ok=True)
