"""BP-C78 free dual flux_threshold elevated vs default (flux rate on). Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        flux_plasticity_rate=0.1,
        flux_threshold=5.0 if elev else 2.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 13601 + ti * 223 + int(treat) * 317)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6741,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C78 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C78"
