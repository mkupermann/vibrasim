"""BP-C32 free dual elevated r_1 vs default. Headless."""
...
def cfg(seed, r1):
    return WorldConfig(
        ...
        rng_seed=seed, r_1=float(r1), r_2=28., freq_tolerance=0.03,
...
def run_one(seed, ti, treat, t_total):
    w = World(cfg(seed, 12.0 if treat else 5.0))
    rng = np.random.default_rng(seed * 4301 + ti * 83 + int(treat) * 53)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((4801,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C32 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C32"
