"""BP-E193 content cascade fire-select after long idle. Headless."""
...
SEEDS, TRIALS = (5261, 5271), 8
N_TRAIN, N_WRITE, T_PROP, T_IDLE = 12, 12, 100, 400
NEAR = 7.0
...
def main(argv=None):
    ...
    seeds, trials, t_idle = ((5261,), 2, 100) if args.smoke else (SEEDS, TRIALS, T_IDLE)
    print(f"BP-E193 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 6601 + ti * 1)
            w = World(cfg(seed))
            train_both(w, rng)
            idle(w, t_idle)
            clear_state(w)
    ...
    result = {"id": "BP-E193", "bars": {
        "B1_path0_after_idle": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_path1_after_idle": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_both": {"value": a3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E193"
