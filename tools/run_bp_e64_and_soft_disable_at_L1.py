"""BP-E64 soft-weaken one AND input at L1 endpoint. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((1911,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E64 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*87053+ti*433)
    ...
    result={"id":"BP-E64","bars":{
        "B1_both_on":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_soft_off":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_restore_on":{"value":a3,"threshold":0.85,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E64"; out.mkdir(parents=True,exist_ok=True)
