"""BP-E75 dual soft-cut AND+OR then selective AND-only restore. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((2191,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E75 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*98089+ti*503)
            ...
            b3s.append(peak_R(w,[L3])<=0.25)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.90,a2>=0.85,a3>=0.90
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E75","bars":{
        "B1_both_off":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_and_restore":{"value":a2,"threshold":0.85,"pass":p2},
        "B3_bypass_still_off":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E75"; out.mkdir(parents=True,exist_ok=True)
