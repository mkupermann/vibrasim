"""BP-C37 free dual reduced global speed vs default. Headless."""
...
def cfg(seed, sp_max):
    return WorldConfig(
        ...
        lambda_gen=0., lambda_dec=0., speed_min=3., speed_max=float(sp_max),
        midplane_wall_enabled=True, midplane_wall_x=MID,
        repulsion_cell_size=100.0,
    )

def inject(w, rng, n, x0, x1, f0, f1, sp0, sp1):
...
def run_one(seed, ti, treat, t_total):
    w = World(cfg(seed, 10.0 if treat else 25.0))
    rng = np.random.default_rng(seed * 5301 + ti * 41 + int(treat) * 73)
    if treat:
        inject(w, rng, N_SIDE, 8, 32, LOW[0], LOW[1], 3.0, 10.0)
        inject(w, rng, N_SIDE, 48, 72, HIGH[0], HIGH[1], 3.0, 10.0)
    else:
        inject(w, rng, N_SIDE, 8, 32, LOW[0], LOW[1], 5.0, 25.0)
        inject(w, rng, N_SIDE, 48, 72, HIGH[0], HIGH[1], 5.0, 25.0)
    evolve(w, t_total)
    return sides(w)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5001,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C37 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C37"
