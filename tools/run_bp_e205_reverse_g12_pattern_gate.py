"""BP-E205 reverse fire-select under G12 pattern gate. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((5761,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E205 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 8901 + ti * 51)
            w = World(cfg(seed))
            train_both(w, rng)
            tag_both(w)
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E205"
