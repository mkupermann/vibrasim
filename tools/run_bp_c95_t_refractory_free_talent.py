"""BP-C95 free dual t_refractory elevated vs default. Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        neuron_dynamics_enabled=True, theta_fire=2., n_emit=0,
        t_refractory=0.20 if elev else 0.05,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 21801 + ti * 347 + int(treat) * 443)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7581,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C95 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C95"
