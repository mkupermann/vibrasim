"""BP-E102 soft 2x2 full restore then hard re-cut arm 00. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((2901,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E102 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*125183+ti*677)
    ...
    result={"id":"BP-E102","bars":{
        "B1_L0_fanout":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_hard_cut00":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_L1_both":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E102"; out.mkdir(parents=True,exist_ok=True)
