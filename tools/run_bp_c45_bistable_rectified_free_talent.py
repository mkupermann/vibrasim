"""BP-C45 free dual bistable_drive_rectified vs off. Headless."""
...
def cfg(seed, rect):
    return WorldConfig(
        ...
        bistable_drive_rectified=bool(rect),
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 7001 + ti * 19 + int(treat) * 113)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5381,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C45 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C45"
