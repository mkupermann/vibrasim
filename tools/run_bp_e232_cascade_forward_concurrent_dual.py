"""BP-E232 cascade forward concurrent dual L fire. Headless."""
...
def fire_near_both(w, pos_a, pos_b, n=T_PROP):
    thr = float(w.config.theta_fire)
    dt = float(w.config.dt)
    for t in range(n):
        if t % 6 == 0:
...
def both_r_lit(w):
    return peak_near(w, R0) >= 1.0 and peak_near(w, R1) >= 1.0

def main(argv=None):
    p = argparse.ArgumentParser(); p.add_argument("--smoke", action="store_true")
    args = p.parse_args(argv)
    seeds, trials = ((6961,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E232 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 15601 + ti * 151)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_near(w, L0)
            b1s.append(select_p0(w))
            clear_state(w)
            fire_near(w, L1)
            b2s.append(select_p1(w))
            clear_state(w)
            fire_near_both(w, L0, L1)
            b3s.append(both_r_lit(w))
    b1 = float(np.mean(b1s)); b2 = float(np.mean(b2s)); b3 = float(np.mean(b3s))
    p1, p2, p3 = b1 >= 0.90, b2 >= 0.90, b3 >= 0.70
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E232", "bars": {
        "B1_seq_sel_p0": {"value": b1, "threshold": 0.90, "pass": p1},
        "B2_seq_sel_p1": {"value": b2, "threshold": 0.90, "pass": p2},
        "B3_concurrent_both_r": {"value": b3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E232"
