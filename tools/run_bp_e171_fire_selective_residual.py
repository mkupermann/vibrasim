"""BP-E171 fire-readout selective residual freq-matched L fire. Headless."""
...
LOG_TOL = 0.35  # log-space match for freq-selective fire
...
def main(argv=None):
    ...
    seeds, trials = ((4541,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E171 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 3001 + ti * 139)
            w = World(cfg(seed))
            train_both(w, rng)
            # Probe A
            w.k_charge[:] = 0
            ...
            b1s.append(ok_a)
            # Probe B on fresh train
            rng2 = np.random.default_rng(seed * 3001 + ti * 139 + 11)
            w2 = World(cfg(seed + 7))
            train_both(w2, rng2)
            w2.k_charge[:] = 0
            if hasattr(w2, "k_latch"):
                w2.k_latch[:] = 0
            fire_L_band(w2, F_HI)
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E171"
