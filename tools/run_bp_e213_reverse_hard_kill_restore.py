"""BP-E213 reverse split hard-kill L1 then retrain restore. Headless."""
...
def train_arm(w, rng, L, R, fL, fR, pid, n_train=N_TRAIN):
    w.active_pattern_id = pid
...
def train_both(w, rng):
    train_arm(w, rng, L0, R0, F_LO, F_HI, 1)
    train_arm(w, rng, L1, R1, F_HI, F_LO, 2)
...
def main(argv=None):
    ...
    seeds, trials = ((6081,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E213 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 10501 + ti * 67)
            w = World(cfg(seed))
            train_both(w, rng)
            hard_cut_at(w, L1)
            idle(w, 10)
            ...
            clear_state(w)
            fire_near(w, R1)
            b1s.append(not reverse_c1(w))
            w.active_pattern_id = 1
            clear_state(w)
            fire_near(w, R0)
            b2s.append(reverse_c0(w))
            train_arm(w, rng, L1, R1, F_HI, F_LO, 2, n_train=N_TRAIN)
            idle(w, 10)
            w.active_pattern_id = 2
            clear_state(w)
            fire_near(w, R1)
            b3s.append(reverse_c1(w))
    a1, a2, a3 = map(float, (np.mean(b1s), np.mean(b2s), np.mean(b3s)))
    p1, p2, p3 = a1 >= 0.70, a2 >= 0.80, a3 >= 0.80
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E213", "bars": {
        "B1_post_hard_rev_fail": {"value": a1, "threshold": 0.70, "pass": p1},
        "B2_rev_pid1_survives": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_restore_rev_pid2": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E213"
