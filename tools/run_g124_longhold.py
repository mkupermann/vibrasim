"""G124 — long-term stability of the WORKING memory (full atom-clearing maintenance) at POST=2500.
...
def cycle(w, c, snap, pattern):
    ...
    for ci in assign:
        if w.k_alive[ci]:
            w.k_vel[ci] = 0.0
    for _ in range(POST_T):                    # HOLD with FULL atom-clearing maintenance
        clear_band_atoms_except(w, keep)
        tick(w, c.dt)
...
if __name__ == "__main__":
    print(f"=== G124: long-term stability, FULL maintenance, POST={POST_T} ===", flush=True)
    okW = okC = True
    for s in [42, 7]:
        wr, ctrl = run(s)
    ...
    passed = okW and okC
    print("\n--- VERDICT ---", flush=True)
    print(f"G124a pattern held at POST=2500 (both seeds): {okW}", flush=True)
    print(f"G124b control empty (both seeds)            : {okC}", flush=True)
    print(("G124: PASS - the working (full-maintenance) matter memory is robustly persistent long-term (POST=2500)"
           if passed else "G124: NULL/PARTIAL - degrades at the longer hold (see readouts)"), flush=True)
