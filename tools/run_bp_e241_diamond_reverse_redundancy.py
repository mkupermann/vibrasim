"""BP-E241 diamond reverse redundancy (no mid-kill). Headless."""
...
SEEDS, TRIALS = (7281, 7291), 6
N_WRITE, T_PROP = 12, 100
NEAR = 8.0
L = np.array([12., 25., 25.])
M1 = np.array([40., 15., 25.])
M2 = np.array([40., 35., 25.])
R = np.array([68., 25., 25.])
...
def fire_R(w, n=T_PROP):
    thr = float(w.config.theta_fire)
    dt = float(w.config.dt)
    for t in range(n):
        if t % 6 == 0:
            for i in range(w.k_count):
                if w.k_alive[i] and int(w.k_level[i]) >= 4 and float(np.linalg.norm(w.k_pos[i] - R)) <= NEAR:
                    w.k_charge[i] = thr + 5.
        tick(w, dt); w.t += dt
...
def main(argv=None):
    ...
    seeds, trials = ((7281,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E241 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 18801 + ti * 197)
            w = World(cfg(seed))
            train_full(w, rng)
            clear_state(w)
            fire_R(w)
            ...
            w2 = World(cfg(seed + 3))
            rng2 = np.random.default_rng(seed * 18801 + ti * 197 + 3)
            train_single(w2, rng2)
            clear_state(w2)
            fire_R(w2)
            b2s.append(peak_L(w2) >= 1.0)
            w3 = World(cfg(seed + 7))
            clear_state(w3)
            fire_R(w3)
            b3s.append(peak_L(w3) < 1.0)
    b1 = float(np.mean(b1s)); b2 = float(np.mean(b2s)); b3 = float(np.mean(b3s))
    p1, p2, p3 = b1 >= 0.90, b2 >= 0.80, b3 >= 0.70
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E241", "bars": {
        "B1_full_diamond": {"value": b1, "threshold": 0.90, "pass": p1},
        "B2_single_arm": {"value": b2, "threshold": 0.80, "pass": p2},
        "B3_no_train_fail": {"value": b3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E241"
