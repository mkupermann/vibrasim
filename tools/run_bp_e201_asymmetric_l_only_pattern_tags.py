"""BP-E201 asymmetric L-only pattern tags G12 wrong-arm block. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((5601,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E201 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 8101 + ti * 41)
            w = World(cfg(seed))
            train_both(w, rng)
            tag_l_only(w)
    ...
    result = {"id": "BP-E201", "bars": {
        "B1_correct_L_only": {"value": a1, "threshold": 0.90, "pass": p1},
        "B2_wrong_arm_L_only": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_both_end_wrong_fail": {"value": a3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E201"
