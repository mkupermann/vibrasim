"""BP-E148 hybrid cascade multi-trial hard path-switch OR-AND-OR. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3961,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E148 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*171217+ti*1019)
    ...
    result={"id":"BP-E148","bars":{
        "B1_or_only":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_cascade_and_only":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_or_only_again":{"value":a3,"threshold":0.75,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E148"; out.mkdir(parents=True,exist_ok=True)
