"""BP-C74 free dual synaptic_transmission_strength elevated vs default. Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        synaptic_transmission_strength=2.0 if elev else 0.5,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 12801 + ti * 191 + int(treat) * 293)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6581,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C74 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C74"
