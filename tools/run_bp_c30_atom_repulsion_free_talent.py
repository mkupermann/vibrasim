"""BP-C30 free dual elevated atom_repulsion_k vs default 0. Headless."""
...
SEEDS, TRIALS = (4681, 4691), 2
MID = 40.0
LOW, HIGH = (100.0, 2000.0), (500.0, 10000.0)
ATOM_REP_TREAT, ATOM_REP_CTRL = 20.0, 0.0

def cfg(seed, atom_rep):
    return WorldConfig(
        ...
        atom_repulsion_k=float(atom_rep),
    )
...
def run_one(seed, ti, treat, t_total):
    w = World(cfg(seed, ATOM_REP_TREAT if treat else ATOM_REP_CTRL))
    rng = np.random.default_rng(seed * 3701 + ti * 109 + int(treat) * 43)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((4681,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C30 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C30"
