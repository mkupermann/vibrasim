"""BP-E83 soft 2x2 dual-cut, swap restore, then identity reconfig. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((2501,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E83 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*106111+ti*569)
            ...
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0])
            b2s.append(end_near(w,R1)>=1.0 and end_near(w,R0)<=0.25)
            for k in ("01","10"):
                soft_cut_key(w,k)
            for k in ("00","11"):
                restore(w,rng,k)
    ...
    result={"id":"BP-E83","bars":{
        "B1_both_off":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_swap":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_identity":{"value":a3,"threshold":0.75,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E83"; out.mkdir(parents=True,exist_ok=True)
