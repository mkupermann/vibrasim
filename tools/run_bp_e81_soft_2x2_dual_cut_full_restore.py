"""BP-E81 soft 2x2 dual-cut all then full restore four arms. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((2461,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E81 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*104107+ti*557)
            ...
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L0])
            b3s.append(end_near(w,R0)>=1.0 and end_near(w,R1)>=1.0)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.80
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E81","bars":{
        "B1_both_off":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_full_restore":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_L0_fanout":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E81"; out.mkdir(parents=True,exist_ok=True)
