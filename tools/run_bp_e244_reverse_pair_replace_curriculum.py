"""BP-E244 reverse cascade pair_replace curriculum: p1 train may break p0 reverse. Headless."""
...
def cfg(seed):
    return WorldConfig(
        ...
        ilw_pair_link_enabled=True, ilw_pair_link_delta=1.0, ilw_pair_replace_enabled=True,
...
def main(argv=None):
    ...
    seeds, trials = ((7341,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E244 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 19401 + ti * 223)
            w = World(cfg(seed))
            train_p0(w, rng)
            clear_state(w)
            fire_near(w, R0)
            ...
            clear_state(w)
            fire_near(w, R1)
            b2s.append(reverse_p1(w))
            clear_state(w)
            fire_near(w, R0)
            b3s.append(not reverse_p0(w))
    b1 = float(np.mean(b1s)); b2 = float(np.mean(b2s)); b3 = float(np.mean(b3s))
    p1, p2, p3 = b1 >= 0.90, b2 >= 0.90, b3 >= 0.70
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E244", "bars": {
        "B1_p0_only_rev": {"value": b1, "threshold": 0.90, "pass": p1},
        "B2_after_p1_rev_p1": {"value": b2, "threshold": 0.90, "pass": p2},
        "B3_after_p1_rev_p0_fails": {"value": b3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E244"
