"""BP-C104 free dual correlation plasticity (Hebbian, BET-099) ON vs OFF. Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        corr_plasticity_rate=1.0 if elev else 0.0,
    )
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7921,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C104 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C104"
