"""BP-E246 cascade reverse asymmetric heavy retrain path0; path1 survives. Headless."""
...
def train_p0(w, rng, n_train=N_TRAIN):
    for _ in range(n_train):
        link(w, rng, L0, M0, F0L, F0M)
        link(w, rng, M0, R0, F0M, F0R)
        idle(w, 4)

def train_p1(w, rng, n_train=N_TRAIN):
    for _ in range(n_train):
...
def main(argv=None):
    ...
    seeds, trials = ((7381,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E246 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 19801 + ti * 229)
            w = World(cfg(seed))
            train_p0(w, rng, N_TRAIN)
            train_p1(w, rng, N_TRAIN)
            clear_state(w)
            fire_near(w, R1)
            b1s.append(reverse_p1(w))
            # heavy asymmetric retrain path0 only
            train_p0(w, rng, N_TRAIN * 3)
            clear_state(w)
            fire_near(w, R0)
            b2s.append(reverse_p0(w))
            clear_state(w)
            fire_near(w, R1)
            b3s.append(reverse_p1(w))
    b1 = float(np.mean(b1s)); b2 = float(np.mean(b2s)); b3 = float(np.mean(b3s))
    p1, p2, p3 = b1 >= 0.90, b2 >= 0.90, b3 >= 0.80
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E246", "bars": {
        "B1_equal_rev_p1": {"value": b1, "threshold": 0.90, "pass": p1},
        "B2_heavy_p0_rev": {"value": b2, "threshold": 0.90, "pass": p2},
        "B3_p1_survives_heavy_p0": {"value": b3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E246"
