"""BP-C15 free dual talent with bridge_consolidate vs control. Headless."""
...
def cfg(seed, consol):
    return WorldConfig(
        ...
        bridge_consolidate_threshold=float(CONSOL if consol else 0.0),
    )
...
def run_one(seed, ti, consol, t_total):
    w=World(cfg(seed, consol))
    rng=np.random.default_rng(seed*2309+ti*71+int(consol)*19)
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((2101,),1,300) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C15 start smoke={args.smoke}")
    on,off=[],[]
    for s in seeds:
    ...
    result={"id":"BP-C15","bars":{
        "B1_consol_spec":{"value":b1,"threshold":0.90,"pass":p1},
        "B2_ctrl_spec":{"value":b2,"threshold":0.80,"pass":p2},
        "B3_consol_pop":{"value":b3,"threshold":0.80,"pass":p3},
        "B4_delta":{"value":b4,"threshold":0.15,"pass":p4},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-C15"; out.mkdir(parents=True,exist_ok=True)
