"""BP-E178 multi-trial split-port arm switch kill/restore. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((4741,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E178 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 4001 + ti * 101)
            w = World(cfg(seed))
            train_both(w, rng)
            hard_cut_at(w, R0)
            idle(w, 10)
    ...
    result = {"id": "BP-E178", "bars": {
        "B1_post_kill_c1": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_restore_c0": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_c1_after_restore": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E178"
