"""BP-E76 dual soft-cut then selective OR-only restore. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((2211,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E76 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*99091+ti*509)
    ...
    result={"id":"BP-E76","bars":{
        "B1_both_off":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_or_restore":{"value":a2,"threshold":0.85,"pass":p2},
        "B3_and_still_off":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E76"; out.mkdir(parents=True,exist_ok=True)
