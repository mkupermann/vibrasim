"""BP-C82 free dual tau_membrane elevated vs default. Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        tau_membrane=2.0 if elev else 0.5,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 14601 + ti * 257 + int(treat) * 359)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6861,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C82 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C82"
