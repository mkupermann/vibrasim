"""BP-C86 free dual freq_ratio elevated vs default. Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        freq_ratio=0.25 if elev else 0.08,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 16201 + ti * 281 + int(treat) * 383)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7021,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C86 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C86"
