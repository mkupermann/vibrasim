"""BP-E140 hybrid hard multi-trial path-switch AND-OR-AND. Headless."""
...
I1=np.array([12.,18.,30.]); I3=np.array([12.,25.,45.])
...
def main(argv=None):
    ...
    seeds,trials=((3761,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E140 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*163217+ti*967)
            w=World(cfg(seed)); train(w,rng)
            hard_cut_at(w,I3)
            restore_and(w,rng)
            b1s.append(and_only(w))
            hard_cut_at(w,I1)
            restore_or(w,rng)
            b2s.append(or_only(w))
            hard_cut_at(w,I3)
            restore_and(w,rng)
            b3s.append(and_only(w))
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.75
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E140","bars":{
        "B1_and_only":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_or_only":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_and_only_again":{"value":a3,"threshold":0.75,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E140"; out.mkdir(parents=True,exist_ok=True)
