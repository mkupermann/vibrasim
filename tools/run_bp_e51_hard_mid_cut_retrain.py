"""BP-E51 hard mid-hop cut + A-B retrain only. Headless."""
...
SEEDS, TRIALS = (1541, 1551), 8
N_WRITE, T_PROP, N_RES = 10, 50, 8
L=np.array([10.,25.,25.]); A=np.array([28.,25.,25.])
B=np.array([50.,25.,25.]); R=np.array([68.,25.,25.])
I=np.array([39.,25.,25.])  # midpoint A-B
...
def train(w,rng):
    ...
    for i in range(w.k_count):
        if w.k_alive[i] and int(w.k_level[i])>=4 and float(np.linalg.norm(w.k_pos[i]-I))<=5:
            w.k_kill_bridge_emitter[i]=1

def restore_AB(w,rng):
    link(w,rng,A,B,1000.,2500.,N_RES)
...
def main(argv=None):
    ...
    seeds,trials=((1541,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E51 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*74003+ti*353)
    ...
    result={"id":"BP-E51","bars":{
        "B1_on":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_cut":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_restore_AB":{"value":a3,"threshold":0.85,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E51"; out.mkdir(parents=True,exist_ok=True)
