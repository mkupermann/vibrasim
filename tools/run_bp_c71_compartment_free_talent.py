"""BP-C71 free dual compartment_k vs off. Headless."""
...
def cfg(seed, comp):
    return WorldConfig(
        ...
        compartment_k=1.0 if comp else 0.0,
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 12201 + ti * 163 + int(treat) * 269)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6451,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C71 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C71"
