"""BP-E191 incomplete cascade missing last hop negative control. Headless."""
...
def train_incomplete_p0_full_p1(w, rng):
    for _ in range(N_TRAIN):
        link(w, rng, L0, M0, F0L, F0M)  # no M0-R0
        link(w, rng, L1, M1, F1L, F1M)
        link(w, rng, M1, R1, F1M, F1R)
        idle(w, 4)

def train_full_p0(w, rng):
    for _ in range(N_TRAIN):
...
def main(argv=None):
    ...
    seeds, trials = ((5141,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E191 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 6001 + ti * 13)
            w = World(cfg(seed))
            train_incomplete_p0_full_p1(w, rng)
            clear_state(w)
            fire_near(w, L0)
            b1s.append(not select_p0(w))
            clear_state(w)
            fire_near(w, L1)
            b2s.append(select_p1(w))
            rng2 = np.random.default_rng(seed * 6001 + ti * 13 + 7)
            w2 = World(cfg(seed + 3))
            train_full_p0(w2, rng2)
            clear_state(w2)
            fire_near(w2, L0)
            # only path0 present; select if R0 >= 1
            b3s.append(peak_near(w2, R0) >= 1.0)
    a1, a2, a3 = map(float, (np.mean(b1s), np.mean(b2s), np.mean(b3s)))
    p1, p2, p3 = a1 >= 0.80, a2 >= 0.80, a3 >= 0.80
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E191", "bars": {
        "B1_incomplete_p0_fail": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_full_p1_select": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_full_p0_control": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E191"
