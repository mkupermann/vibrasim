"""BP-C83 free dual r_integrate elevated vs default. Headless."""
...
def cfg(seed, elev):
    return WorldConfig(
        ...
        r_integrate=12.0 if elev else 5.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 15001 + ti * 263 + int(treat) * 367)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6901,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C83 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C83"
