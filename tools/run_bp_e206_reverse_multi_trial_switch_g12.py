"""BP-E206 reverse multi-trial switch under G12. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((5801,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E206 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 9101 + ti * 53)
            w = World(cfg(seed))
            train_both(w, rng)
            tag_both(w)
    ...
    result = {"id": "BP-E206", "bars": {
        "B1_rev_c0": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_rev_c1": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_rev_c0_again": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E206"
