...
def tree_dist(parent,a,b):
    ...
    return 999
def main():
    print("=== JEP-54: concept formation - discover hierarchy from noisy features ===", flush=True)
    ...
    base={l: sum(feat[a] for a in ancestors(parent,l)) for l in leaves}
    ...
    if lc>=0.8 and lp>=0.9:
        print(f"JEP-54: PASS - concept formation WORKS: from noisy feature observations, agglomerative clustering", flush=True)
        ...
        print(f"JEP-54: PARTIAL/NULL - low-noise cophenetic {lc:.2f}, purity {lp:.2f}", flush=True)
