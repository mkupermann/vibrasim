"""BP-E229 cascade reverse incomplete-path negative (no M-R hops). Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((6841,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E229 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 14401 + ti * 131)
            # Incomplete: L-M only
            w = World(cfg(seed))
            train_lm_only(w, rng)
            clear_state(w)
            ...
            # Full-path control (fresh world)
            w2 = World(cfg(seed + 17))
            rng2 = np.random.default_rng(seed * 14401 + ti * 131 + 7)
            train_full(w2, rng2)
            clear_state(w2)
            fire_near(w2, R0)
            b3s.append(reverse_p0(w2))
    b1 = float(np.mean(b1s)); b2 = float(np.mean(b2s)); b3 = float(np.mean(b3s))
    p1, p2, p3 = b1 >= 0.70, b2 >= 0.70, b3 >= 0.90
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E229", "bars": {
        "B1_incomplete_r0_fail": {"value": b1, "threshold": 0.70, "pass": p1},
        "B2_incomplete_r1_fail": {"value": b2, "threshold": 0.70, "pass": p2},
        "B3_full_r0_ok": {"value": b3, "threshold": 0.90, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E229"
