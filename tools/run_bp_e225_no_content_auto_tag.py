"""BP-E225 no content auto-tag without active_pattern_id. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((6641,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E225 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 13101 + ti * 107)
            w = World(cfg(seed))
            train_ambient(w, rng)
            b1s.append(tag_frac(w) <= 0.05)
            w.active_pattern_id = 2
            clear_state(w)
            fire_L_band(w, F_LO)
            b2s.append(select_c0(w))  # wrong-arm still selects
            tag_both(w)
            w.active_pattern_id = 2
            clear_state(w)
            fire_L_band(w, F_LO)
            b3s.append(not select_c0(w))
    a1, a2, a3 = map(float, (np.mean(b1s), np.mean(b2s), np.mean(b3s)))
    p1, p2, p3 = a1 >= 0.90, a2 >= 0.70, a3 >= 0.70  # B1 is fraction of trials with tag_frac<=0.05
    # Wait B1 bars: tag fraction <=0.05 - I used mean of booleans for trials
    # Better report mean tag_frac as well - for bars: fraction of trials where tag_frac<=0.05 >=0.90
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E225", "bars": {
        "B1_no_auto_tags": {"value": a1, "threshold": 0.90, "pass": p1},
        "B2_wrong_still_selects": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_posthoc_wrong_fail": {"value": a3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E225"
