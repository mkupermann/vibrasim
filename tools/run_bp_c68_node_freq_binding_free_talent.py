"""BP-C68 free dual node_freq_binding OFF vs ON. Headless."""
...
def run_one(seed, ti, treat_off, t_total):
    # treat = node_freq_binding OFF; ctrl = ON
    freq_bind = not treat_off
    w = World(cfg(seed, freq_bind))
    rng = np.random.default_rng(seed * 11801 + ti * 149 + int(treat_off) * 251)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6341,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C68 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C68"
