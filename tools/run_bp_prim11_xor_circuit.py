"""PRIM11-D0 + XOR circuit: OR path + AND latch-clear. Headless."""
...
SEEDS, TRIALS = (1231, 1241), 10
N_WRITE, T_PROP = 12, 80
L1=np.array([12.,18.,25.]); L2=np.array([12.,32.,25.])
Mor=np.array([35.,25.,25.]); Mand=np.array([40.,40.,25.]); R=np.array([65.,25.,25.])
...
def cfg(seed):
    return WorldConfig(
        ...
        coincidence_and_enabled=True, fire_zero_latch_radius=0.0,  # set only on Mand nodes
    )
...
def train(w,rng):
    # OR path
    link(w,rng,L1,Mor,400.,1500.); link(w,rng,L2,Mor,800.,1500.); link(w,rng,Mor,R,1500.,5000.)
    # AND path to Mand (coincidence) — Mand will zero R latch when it fires
    link(w,rng,L1,Mand,400.,2000.); link(w,rng,L2,Mand,800.,2000.)
    # tag Mand: coincidence gate + zero-latch emitter covering R
    for i in range(w.k_count):
        if w.k_alive[i] and int(w.k_level[i])>=4 and float(np.linalg.norm(w.k_pos[i]-Mand))<=8:
            w.k_coincidence_gate[i] = 1
            w.k_zero_latch_emitter[i] = 1
    object.__setattr__(w.config, "fire_zero_latch_radius", 45.0)
...
def main(argv=None):
    ...
    seeds,trials=((1231,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"PRIM11-D0/XOR start smoke={args.smoke}")
    x1s,x2s,x3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*60053+ti*269)
            w=World(cfg(seed)); train(w,rng)
            x1s.append(end_R(w,[L1])>=1.0)
            w2=World(cfg(seed)); train(w2,rng)
            x2s.append(end_R(w2,[L2])>=1.0)
            w3=World(cfg(seed)); train(w3,rng)
            x3s.append(end_R(w3,[L1,L2])<=0.25)
    a1,a2,a3=map(float,(np.mean(x1s),np.mean(x2s),np.mean(x3s)))
    p1,p2,p3=a1>=0.85,a2>=0.85,a3>=0.85
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"PRIM11-D0","bars":{
        "X1_L1":{"value":a1,"threshold":0.85,"pass":p1},
        "X2_L2":{"value":a2,"threshold":0.85,"pass":p2},
        "X3_both_off":{"value":a3,"threshold":0.85,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"PRIM11-D0"; out.mkdir(parents=True,exist_ok=True)
