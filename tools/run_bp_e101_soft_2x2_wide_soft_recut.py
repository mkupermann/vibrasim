"""BP-E101 soft 2x2 wide-sep full restore + soft re-cut arm 00. Headless."""
...
SEEDS, TRIALS = (2881, 2891), 6
N_WRITE, T_PROP, N_RES = 10, 45, 6
# wide y-sep L0/R0 at 12, L1/R1 at 40 — mid row spacing large
...
def main(argv=None):
    ...
    seeds,trials=((2881,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E101 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*124181+ti*673)
    ...
    result={"id":"BP-E101","bars":{
        "B1_L0_fanout":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_cut00":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_L1_both":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E101"; out.mkdir(parents=True,exist_ok=True)
