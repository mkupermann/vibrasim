"""BP-E62 soft-disable one arm of coincidence AND. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((1811,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E62 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*85043+ti*421)
    ...
    result={"id":"BP-E62","bars":{
        "B1_both_on":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_disable_off":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_restore_on":{"value":a3,"threshold":0.85,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E62"; out.mkdir(parents=True,exist_ok=True)
