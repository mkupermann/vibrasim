"""BP-E66 soft-cut OR bypass of AND-OR hybrid. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((1951,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E66 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*89059+ti*443)
            w=World(cfg(seed)); train(w,rng)
            soft_cut_L3(w)
            b1s.append(peak_R(w,[L1,L2])>=1.0)
            b2s.append(peak_R(w,[L3])<=0.25)
            b3s.append(peak_R(w,[L1])<=0.25)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.90,a2>=0.90,a3>=0.90
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E66","bars":{
        "B1_and_after_cut":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_bypass_off":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_L1_only_off":{"value":a3,"threshold":0.90,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E66"; out.mkdir(parents=True,exist_ok=True)
