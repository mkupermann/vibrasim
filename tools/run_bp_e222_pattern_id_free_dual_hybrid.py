"""BP-E222 pattern-id G12 + free dual C16 strength-decay hybrid. Headless."""
...
def run_one(seed, ti, gate, t_total):
    ...
    rng = np.random.default_rng(seed * 12501 + ti * 173 + int(gate) * 277)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6511,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-E222 start smoke={args.smoke}", flush=True)
    on, off = [], []  # on=treat gate ON, off=ctrl gate OFF
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-E222"
