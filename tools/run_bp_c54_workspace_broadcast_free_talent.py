"""BP-C54 free dual workspace_broadcast OFF vs ON. Headless."""
...
SEEDS, TRIALS = (5781, 5791), 2
MID = 40.0
LOW, HIGH = (100.0, 2000.0), (500.0, 10000.0)

def cfg(seed, broadcast_on):
    return WorldConfig(
        ...
        workspace_broadcast_enabled=bool(broadcast_on),
    )
...
def run_one(seed, ti, treat_off, t_total):
    # treat = broadcast OFF (new ablation); ctrl = broadcast ON
    broadcast_on = not treat_off
    w = World(cfg(seed, broadcast_on))
    rng = np.random.default_rng(seed * 9001 + ti * 79 + int(treat_off) * 167)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5781,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C54 start smoke={args.smoke}", flush=True)
    on, off = [], []  # on=treat (broadcast OFF), off=ctrl (broadcast ON)
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C54"
