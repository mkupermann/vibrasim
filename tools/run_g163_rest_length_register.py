...
Pre-registered in docs/amendments/g163_rest_length_register_corrected.md.
Metrics only; verdict against the frozen bars.

Usage: python tools/run_g163_rest_length_register.py
...
X0 = 15.0
BAND_Y = 30.0
SHORT, LONG = 6.5, 10.5    # bit 0 / bit 1 (G163: SHORT+SHORT=13 >= 12, no skip bonds)
UNIFORM = 8.5              # scramble spacing = encoding midpoint (max ignorance)
T_CONSOL = 8
T_RETRIEVE = 800
N_PATTERNS = 8
TENSION_K = 8.0
...
OUT_DIR = Path(__file__).resolve().parent.parent / "archive" / "run-logs" / "g163"
...
def main():
    ...
    for seed in SEEDS:
        rng = np.random.default_rng(1630 + seed)
