"""BP-E190 triple-hop content cascade L-M-A-R dual path fire-select. Headless."""
...
# path0 ascending; path1 descending content
...
def main(argv=None):
    ...
    seeds, trials = ((5121,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E190 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 5901 + ti * 17)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_near(w, L0)
    ...
    result = {"id": "BP-E190", "bars": {
        "B1_path0_select": {"value": a1, "threshold": 0.80, "pass": p1},
        "B2_path1_select": {"value": a2, "threshold": 0.80, "pass": p2},
        "B3_both": {"value": a3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E190"
