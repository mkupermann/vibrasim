"""BP-C97 free dual freq_distribution linear vs log. Headless."""
...
def cfg(seed, linear):
    return WorldConfig(
        ...
        freq_distribution="linear" if linear else "log",
        freq_min=100.0, freq_max=10000.0,
    )
...
def inject(w, rng, n, x0, x1, f0, f1):
    ...
    for k, i in enumerate(slots):
        ...
        # sample freq according to config distribution for honest knob load
        if str(w.config.freq_distribution).lower() == "linear":
            w.s_freq[i] = float(rng.uniform(f0, f1))
        else:
...
def run_one(seed, ti, treat, t_total):
    w = World(cfg(seed, treat))
    rng = np.random.default_rng(seed * 22601 + ti * 359 + int(treat) * 457)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7661,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C97 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C97"
