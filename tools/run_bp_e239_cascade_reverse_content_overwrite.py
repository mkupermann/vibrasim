"""BP-E239 cascade reverse content overwrite same path (no kill). Headless."""
...
# ascending then descending overwrite
F0A = (400.0, 1200.0, 5000.0)
F0B = (5000.0, 1200.0, 400.0)
F1 = (5000.0, 2000.0, 400.0)
...
def train_p0(w, rng, freqs):
    fl, fm, fr = freqs
    for _ in range(N_TRAIN):
        link(w, rng, L0, M0, fl, fm)
        link(w, rng, M0, R0, fm, fr)
        idle(w, 4)

def train_p1_weak(w, rng):
    for _ in range(6):
        link(w, rng, L1, M1, F1[0], F1[1], n=4)
        link(w, rng, M1, R1, F1[1], F1[2], n=4)
        idle(w, 4)
...
def main(argv=None):
    ...
    seeds, trials = ((7241,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E239 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 18401 + ti * 191)
            w = World(cfg(seed))
            train_p0(w, rng, F0A)
            train_p1_weak(w, rng)
            clear_state(w)
            ...
            # overwrite content on same path — no kill
            train_p0(w, rng, F0B)
            clear_state(w)
            fire_near(w, R0)
            ok = reverse_p0(w)
    ...
    p1, p2, p3 = b1 >= 0.90, b2 >= 0.90, b3 >= 0.80
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E239", "bars": {
        "B1_pre_overwrite": {"value": b1, "threshold": 0.90, "pass": p1},
        "B2_post_overwrite": {"value": b2, "threshold": 0.90, "pass": p2},
        "B3_post_exclusive": {"value": b3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E239"
