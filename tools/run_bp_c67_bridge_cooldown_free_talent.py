"""BP-C67 free dual bridge_cooldown vs off. Headless."""
...
def cfg(seed, cool):
    return WorldConfig(
        ...
        bridge_cooldown=0.5 if cool else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 11601 + ti * 139 + int(treat) * 241)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6301,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C67 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C67"
