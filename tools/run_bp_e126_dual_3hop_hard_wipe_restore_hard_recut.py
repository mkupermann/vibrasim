"""BP-E126 dual 3-hop hard wipe both, full restore, hard re-cut path0. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3441,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E126 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*149217+ti*859)
            ...
            hard_cut_at(w,I0); hard_cut_at(w,I1)
    ...
    result={"id":"BP-E126","bars":{
        "B1_both_initial":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_both_after_hard_wipe_restore":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_hard_recut_p0":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E126"; out.mkdir(parents=True,exist_ok=True)
