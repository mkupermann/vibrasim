"""BP-E152 dual cascade multi-trial hard selective path0 silence-restore cycle. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((4041,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E152 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*175217+ti*1039)
            w=World(cfg(seed)); train(w,rng)
            silence_p0(w)
            b1s.append(path0_off(w) and path1_dual_on(w))
            restore_p0(w,rng)
            b2s.append(path0_dual_on(w) and path1_dual_on(w))
            silence_p0(w)
            b3s.append(path0_off(w) and path1_dual_on(w))
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.80,a2>=0.80,a3>=0.80
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E152","bars":{
        "B1_first_selective":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_restore_both_on":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_second_selective":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E152"; out.mkdir(parents=True,exist_ok=True)
