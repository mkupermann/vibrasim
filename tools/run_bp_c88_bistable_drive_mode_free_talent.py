"""BP-C88 free dual bistable_drive_mode absolute vs relative (rate on both). Headless."""
...
SEEDS, TRIALS = (7101, 7111), 2
MID = 40.0
LOW, HIGH = (100.0, 2000.0), (500.0, 10000.0)

def cfg(seed, absolute):
    return WorldConfig(
        ...
        bistable_rate=0.05,
        bistable_drive_mode="absolute" if absolute else "relative",
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 17001 + ti * 293 + int(treat) * 397)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((7101,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C88 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C88"
