"""BP-E240 concurrent fire L0 (forward) + R1 (reverse) dual-path cross. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((7261,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E240 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 18601 + ti * 193)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            # fire L0 (forward to R0) and R1 (reverse to L1) together
            fire_near_both(w, L0, R1)
            r0 = peak_near(w, R0) >= 1.0
            l1 = peak_near(w, L1) >= 1.0
            b1s.append(r0)
            b2s.append(l1)
            b3s.append(r0 and l1)
    b1 = float(np.mean(b1s)); b2 = float(np.mean(b2s)); b3 = float(np.mean(b3s))
    p1, p2, p3 = b1 >= 0.80, b2 >= 0.80, b3 >= 0.70
    verdict = "PASS" if all([p1, p2, p3]) else "NULL"
    result = {"id": "BP-E240", "bars": {
        "B1_forward_r0": {"value": b1, "threshold": 0.80, "pass": p1},
        "B2_reverse_l1": {"value": b2, "threshold": 0.80, "pass": p2},
        "B3_both": {"value": b3, "threshold": 0.70, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E240"
