"""BP-E71 soft-cut then restore OR bypass of AND-OR hybrid. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((2081,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E71 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*94073+ti*479)
    ...
    result={"id":"BP-E71","bars":{
        "B1_bypass_off":{"value":a1,"threshold":0.90,"pass":p1},
        "B2_and_on":{"value":a2,"threshold":0.90,"pass":p2},
        "B3_bypass_restore":{"value":a3,"threshold":0.85,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E71"; out.mkdir(parents=True,exist_ok=True)
