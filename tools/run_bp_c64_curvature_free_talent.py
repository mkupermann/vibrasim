"""BP-C64 free dual curvature_k vs off. Headless."""
...
def cfg(seed, curv):
    return WorldConfig(
        ...
        curvature_k=2.0 if curv else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 11001 + ti * 127 + int(treat) * 227)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6181,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C64 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C64"
