"""BP-E48 retrain OR path after XOR structural cut. Headless."""
...
SEEDS, TRIALS = (1481, 1491), 8
N_WRITE, T_PROP = 12, 50
...
def main(argv=None):
    ...
    seeds,trials=((1481,),3) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E48 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*71093+ti*337)
            w=World(cfg(seed)); train_xor(w,rng)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L1]); b1s.append(end_R(w)>=1.0)
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L1,L2])  # cut
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L1]); b2s.append(end_R(w)<=0.25)
            retrain_or(w,rng)
            # disarm Mand emitters so retrain not immediately cut
            if hasattr(w,"k_kill_bridge_emitter"):
                w.k_kill_bridge_emitter[:]=0
            if hasattr(w,"k_coincidence_gate"):
                w.k_coincidence_gate[:]=0
            w.k_charge[:]=0; w.k_latch[:]=0
            fire_phase(w,[L1]); b3s.append(end_R(w)>=1.0)
    a1,a2,a3=map(float,(np.mean(b1s),np.mean(b2s),np.mean(b3s)))
    p1,p2,p3=a1>=0.85,a2>=0.85,a3>=0.85
    verdict="PASS" if all([p1,p2,p3]) else "NULL"
    result={"id":"BP-E48","bars":{
        "B1_pre":{"value":a1,"threshold":0.85,"pass":p1},
        "B2_cut":{"value":a2,"threshold":0.85,"pass":p2},
        "B3_retrain":{"value":a3,"threshold":0.85,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E48"; out.mkdir(parents=True,exist_ok=True)
