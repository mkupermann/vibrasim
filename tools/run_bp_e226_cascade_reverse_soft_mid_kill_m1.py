"""BP-E226 cascade reverse soft mid-kill M1; reverse p0 survives. Headless."""
...
def main(argv=None):
    ...
    seeds, trials = ((6681,), 2) if args.smoke else (SEEDS, TRIALS)
    print(f"BP-E226 start smoke={args.smoke}", flush=True)
    b1s, b2s, b3s = [], [], []
    for seed in seeds:
        for ti in range(trials):
            rng = np.random.default_rng(seed * 13301 + ti * 109)
            w = World(cfg(seed))
            train_both(w, rng)
            clear_state(w)
            fire_near(w, R1)
    ...
    result = {"id": "BP-E226", "bars": {
        "B1_pre_rev_p1": {"value": a1, "threshold": 0.90, "pass": p1},
        "B2_post_soft_rev_fail": {"value": a2, "threshold": 0.70, "pass": p2},
        "B3_rev_p0_survives": {"value": a3, "threshold": 0.80, "pass": p3},
    }, "verdict": verdict}
    out = Path.home() / ".eqmod" / "bet" / "BP-E226"
