"""BP-E133 coincidence AND hard dual wipe, restore L1 only then L2. Headless."""
...
def main(argv=None):
    ...
    seeds,trials=((3621,),2) if args.smoke else (SEEDS,TRIALS)
    print(f"BP-E133 start smoke={args.smoke}")
    b1s,b2s,b3s=[],[],[]
    for seed in seeds:
        for ti in range(trials):
            rng=np.random.default_rng(seed*156217+ti*911)
    ...
    result={"id":"BP-E133","bars":{
        "B1_hard_dual_silence":{"value":a1,"threshold":0.80,"pass":p1},
        "B2_L1_only_still_off":{"value":a2,"threshold":0.80,"pass":p2},
        "B3_both_restore_on":{"value":a3,"threshold":0.80,"pass":p3},
    },"verdict":verdict}
    out=Path.home()/".eqmod"/"bet"/"BP-E133"; out.mkdir(parents=True,exist_ok=True)
