"""BP-C60 free dual bistable_rate vs off. Headless."""
...
def cfg(seed, bist):
    return WorldConfig(
        ...
        bistable_rate=0.05 if bist else 0.0,
    )
...
def run_one(seed, ti, treat, t_total):
    ...
    rng = np.random.default_rng(seed * 10201 + ti * 103 + int(treat) * 197)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6021,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C60 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C60"
