"""G171 — association bandwidth law: contact points k as the variable.

Pre-registered in docs/amendments/g171_association_bandwidth.md.
Metrics only; verdict against the frozen bars. Resumable per arm.

Usage: python tools/run_g171_association_bandwidth.py
...
TENSION_K = 8.0
DAMPING = 0.95
Y_A, Y_B = 25.0, 35.0        # groups 10 apart (ends in window for cross-write)
CHAIN_DY = 14.0              # chains WITHIN a group: y-offset > 12 (no intra-group cross)
OUT_DIR = Path(__file__).resolve().parent.parent / "archive" / "run-logs" / "g171"
...
def run_one(patA, patB, seed: int, m: int, mode: str) -> dict:
    ...
    if mode == "scramx":
        ...
        while True:
            decoy = list(rng_d.integers(0, 2, BITS))
            if 0 < sum(decoy) < BITS and decoy != patB:
                decoyB = decoy
                break
...
ARMS = [("K2", 1, "assoc"), ("K6", 3, "assoc"), ("K12", 6, "assoc"),
...
def main():
    ...
    for name, m, mode in ARMS:
        ...
        cross_tot = 0
        for seed in SEEDS:
            rng = np.random.default_rng(1710 + seed)
            a_sum = 0.0
