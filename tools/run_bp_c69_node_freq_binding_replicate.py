"""BP-C69 larger-N replicate C68 node_freq_binding OFF free dual. Headless."""
...
SEEDS, TRIALS = (6361, 6371, 6381, 6391, 6401), 3
MID = 40.0
LOW, HIGH = (100.0, 2000.0), (500.0, 10000.0)

def cfg(seed, freq_bind):
    return WorldConfig(
        ...
        node_freq_binding=bool(freq_bind),
    )
...
def run_one(seed, ti, treat_off, t_total):
    freq_bind = not treat_off
    w = World(cfg(seed, freq_bind))
    rng = np.random.default_rng(seed * 11901 + ti * 151 + int(treat_off) * 257)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((6361,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C69 start smoke={args.smoke} seeds={len(seeds)} trials={trials}", flush=True)
    on, off = [], []
    for s in seeds:
