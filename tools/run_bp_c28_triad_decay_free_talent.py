"""BP-C28 free dual short triad_decay_time vs default; budget-fit. Headless."""
...
SEEDS, TRIALS = (4361, 4371), 2
MID = 40.0
LOW, HIGH = (100.0, 2000.0), (500.0, 10000.0)
TAU_TREAT, TAU_CTRL = 80.0, 600.0

def cfg(seed, triad_decay):
    return WorldConfig(
        ...
        pair_decay_time=60., triad_decay_time=float(triad_decay),
...
def run_one(seed, ti, treat, t_total):
    ...
    rng=np.random.default_rng(seed*2201+ti*181+int(treat)*37)
...
def main(argv=None):
    ...
    seeds,trials,t_tot=((4361,),1,200) if args.smoke else (SEEDS,TRIALS,T_FULL)
    print(f"BP-C28 start smoke={args.smoke}", flush=True)
    on,off=[],[]
    for s in seeds:
    ...
    out=Path.home()/".eqmod"/"bet"/"BP-C28"; out.mkdir(parents=True,exist_ok=True)
