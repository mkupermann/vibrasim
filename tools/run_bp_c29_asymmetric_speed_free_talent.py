"""BP-C29 free dual asymmetric L/R speed vs symmetric. Headless."""
...
def cfg(seed):
    return WorldConfig(
        ...
        lambda_gen=0., lambda_dec=0., speed_min=1., speed_max=30.,
...
def run_one(seed, ti, treat, t_total):
    w = World(cfg(seed))
    rng = np.random.default_rng(seed * 3101 + ti * 127 + int(treat) * 41)
    if treat:
        inject(w, rng, N_SIDE, 8, 32, LOW[0], LOW[1], 1.0, 6.0)
        inject(w, rng, N_SIDE, 48, 72, HIGH[0], HIGH[1], 15.0, 30.0)
    else:
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((4561,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C29 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C29"
