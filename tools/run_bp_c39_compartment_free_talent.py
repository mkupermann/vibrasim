"""BP-C39 free dual compartment_boundary vs off. Headless."""
...
SEEDS, TRIALS = (5161, 5171), 2
MID = 40.0
LOW, HIGH = (100.0, 2000.0), (500.0, 10000.0)

def cfg(seed, compartment):
    return WorldConfig(
        ...
        compartment_boundary=float(compartment),
...
def run_one(seed, ti, treat, t_total):
    w = World(cfg(seed, MID if treat else 0.0))
    rng = np.random.default_rng(seed * 6101 + ti * 11 + int(treat) * 83)
...
def main(argv=None):
    ...
    seeds, trials, t_tot = ((5161,), 1, 200) if args.smoke else (SEEDS, TRIALS, T_FULL)
    print(f"BP-C39 start smoke={args.smoke}", flush=True)
    on, off = [], []
    for s in seeds:
    ...
    out = Path.home() / ".eqmod" / "bet" / "BP-C39"
