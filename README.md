...
A hostile reader said: the framing here is doing more work than the substance, and they were right. Two of the actionable points are now addressed in code (`EMERGENCE.json` was renamed to `marker_state.json`; the word "autopoietic" was removed from every code path — `docs/marker_protocol.md` retains it only as a historical correction record). The rest of the criticism stands and is recorded here so the next reader sees both registers at once.
...
**The depth/breadth tradeoff to name out loud:** G14 (BTSP-inspired) + G15 (dream consolidation) + G16 (GNW-flavored conjunction trigger) + G17 (homeostatic parameter feedback driver) + G18 (cross-modal recall) were assembled in a 12-hour session. Each is the thinnest possible operational shell over an idea that has decades of literature. The shells are real; the ideas underneath are not yet implemented at literature depth. The README earlier in this file lists each gap (G14 lacks Magee's instructive higher-order input; G16 lacks gamma-band synchrony / long-distance phase coherence / non-linear ignition; G17 is allopoietic, not autopoietic). These are not framing softeners — they are the actual implementation status.
...
**Six-month engagement check.** If, by **2026-11-19**, this repo has zero stars / zero forks / zero engaged readers, that is the field telling me the framing did not land. Pre-committed response: do not relax the pre-registration discipline to chase appeal. Instead, write a single closing post acknowledging the framing miss, rename the project to reflect the meta-half (process documentation + autopilot lab) as the primary deliverable, and archive the substrate-as-emergent-system narrative as a side study. Decided now while expectations are flat so the decision survives disappointment.

**Citation verification (2026-05-19):** Magee 2026 *Nat Neurosci* and Wu et al 2025 *Nat Comm* (corrected from "2024") both verified via direct web search to exist and say what they're cited for. They are real anchors, not hallucinated. The 2024 → 2025 date for the Wu et al paper has been corrected throughout the codebase.
...
- **Flux** (`world/flux/`, `agent/flux/`) — the project's actual scientific bet. A substrate where the six engineered levels are replaced by one principle: energy quanta flow through an open boundary, structures kondensieren wo sie diesen Fluss effizienter kanalisieren, learning is reconfiguration toward more efficient flux channelling. Spec: [`docs/superpowers/specs/2026-05-10-flux-substrate-design.md`](docs/superpowers/specs/2026-05-10-flux-substrate-design.md). Status as of 2026-05-14: F0 complete (skeleton + energy-conservation audit); F1a complete (binding + T-based decay + T3 crystallization, ratio 9.0); F1b complete (bridges + structure-flux + Hebbian plasticity + node dissociation, T4 decay-without-flux passes); F1c complete-with-caveat (thermal layer + bidirectional injection landed; T2 Bénard test passes for seed=42 with cube=(80,40,10) but pass rate over 10 seeds is 30%, see `docs/flux/phase-log.md` 2026-05-14 entry for the mechanism gap); F2 (cochlea + synthesis + first audio input) is next.
