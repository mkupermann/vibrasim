"""BET-011 — SOM with per-cell saturation. Pre-reg LOGBOOK 2026-05-23 ~21:15.

After BET-009 NULL (SOM 8/9, T8 fail) and BET-010 NULL (SDM 8/9, T8 fail),
the deadlock is T8 catastrophic-forgetting. Diagnostic analysis: T8 with
histogram-KL is biased toward whichever class has broader feature-marginal
distribution; WN's spread across features outweighs EN's concentrated
band-0 contribution, so KL(S_AB vs fresh_WN) is consistently smaller than
KL(S_AB vs fresh_EN) regardless of substrate choice — UNLESS the substrate
explicitly preserves EN's storage cells from WN overwrites.

BET-011 introduces per-cell saturation in a SOM substrate: after a cell's
visit count reaches saturation_threshold=30, the cell becomes
write-protected. Future inputs whose BMU would be that cell are routed
to the next-best unsaturated cell. EN training fills its territory first;
WN training (continuous, no flag) is forced to populate non-EN cells.

Mechanism is local + non-supervised. The substrate decides its own memory
consolidation based on per-cell visit history. No external rehearsal,
no global freeze signal.

Locked T0-T9 bar from LOGBOOK 2026-05-23 ~20:55. saturation_threshold=30
locked pre-data.

Pre-data prediction: T0-T5+T7+T9 PASS (≥ SOM baseline). T8 is the
critical test. If saturation preserves EN-territory cells from WN
overwrites, KL(S_AB vs fresh_EN) should be small (EN-cells essentially
identical between AB and fresh_EN) and KL(S_AB vs fresh_WN) should be
larger (AB has BOTH territories, WN has only WN territory). PASS.
"""
...
from world.flux.som_saturating import (
    SOMSaturatingConfig, evaluate_holdout, initialise, run,
)
...
OUT_DIR = Path.home() / ".eqmod/bet/BET-011"
...
def _write_result_json(verdict, m, audio_meta):
    ...
    payload = {
        "item_id": "BET-011",
        "verdict": verdict,
        "hypothesis": "SOM with per-cell saturation (threshold=30). Self-determined memory consolidation: each cell decides based on visit count when to stop accepting new content.",
        "thresholds": {
...
def _copy_state(state):
    return {
        "w": state["w"].copy(),
        "N": state["N"].copy(),
        "saturated": state["saturated"].copy(),
        "ii": state["ii"],
        "jj": state["jj"],
        "kk": state["kk"],
    }
...
def substrates():
    ...
    if 2 * n_audio > full.shape[0]:
        raise RuntimeError(f"R-7 corpus too short")
    ...
    audio_meta = {
        ...
        "target_rms": TARGET_RMS, "saturation_threshold": cfg.saturation_threshold,
    }
...
def _compute_all(sub):
    ...
    return {
        ...
        "saturation_after_EN_n_saturated": int(sub["state_eng"]["saturated"].sum()),
        "saturation_after_AB_n_saturated": int(sub["state_AB"]["saturated"].sum()),
        "all_nine_pass": all_nine,
    }


def test_som_saturating_passes_T0_to_T9(substrates):
    ...
    if not m["all_nine_pass"]:
        summary = (
            ...
            f"saturated_after_EN={m['saturation_after_EN_n_saturated']}/3600 "
            f"saturated_after_AB={m['saturation_after_AB_n_saturated']}/3600"
        )
        pytest.fail(f"BET-011 NULL: SOM-saturating does not pass T0-T9. {summary}")
