"""BET-028 — canonical comprehensive validation T0-T17.

Single test that runs the WIN-substrate (SOM+replay) once and reports
all 14 validated bars in one result.json. Operator-friendly summary
of the substrate's complete passing profile.

Tests included (all bars locked pre-data from prior LOGBOOK entries):
  T0   spatial std > 0.05
  T1   KL init vs eng > 0.1
  T2   KL eng vs wn > 0.1
  T3   T1+T2 at half corpus
  T4   holdout precision > 0.3
  T5   retention >= 50% after rest
  T7   chunk-shuffle ratio < 0.10
  T8   KL(AB,EN) < KL(AB,WN)
  T9   spatial autocorr > 0.3 AND > 2x fresh
  T13  BMU coverage_EN > 0.10 AND ratio > 2
  T15  E_EN < 0.5 * E_WN  (quantization quality)
  T17  KL(EN-cells, EN-inputs) < 0.5  (histogram fidelity)
"""
...
OUT_DIR = Path.home() / ".eqmod/bet/BET-028"
...
def _compute(sub):
    ...
    return {
        "substrate": "SOM+replay (Kohonen 1982 + Robins 1995)",
        "bars": bars,
        "n_pass": sum(bars.values()),
        "n_total": len(bars),
        "all_pass": all_pass,
        "T0_std": t0_std,
        "T1_kl": kl_t1, "T2_kl": kl_t2,
        "T3_kls": [kl_t1_half, kl_t2_half],
        "T4_precision": holdout["precision"], "T4_cosine": holdout["mean_cosine"],
        "T5_retention": [kl_t1_rest/(kl_t1+1e-9), kl_t2_rest/(kl_t2+1e-9)],
        "T7_ratio": t7_ratio,
        "T8_AB_EN": kl_t8_en, "T8_AB_WN": kl_t8_wn,
        "T8_protection_factor": kl_t8_wn / max(kl_t8_en, 1e-9),
        "T9_autocorr_trained": ac_t, "T9_autocorr_fresh": ac_f,
        "T13_coverage_en": cov_en, "T13_coverage_wn": cov_wn,
        "T13_ratio": ratio_t13,
        "T15_quant_E_EN": sub["quant_e_en"], "T15_quant_E_WN": sub["quant_e_wn"],
        "T15_ratio": t15_ratio,
        "T17_kl_en_cells_vs_en_inputs": kl_t17,
        "T17_kl_wn_cells_vs_en_inputs": kl_t17_neg,
        "T17_discrimination": kl_t17_neg / max(kl_t17, 1e-9),
        "neg_control_kl": neg_kl,
    }


def test_canonical(substrates):
    m = _compute(substrates)
    if not m["all_pass"]:
        failed = [k for k,v in m["bars"].items() if not v]
        pytest.fail(
            f"BET-028 NULL: {m['n_pass']}/{m['n_total']} pass; failed: {failed}"
        )
...
def write_bet_result_at_end(substrates):
    ...
    payload = {
        "item_id": "BET-028",
        "verdict": verdict,
        "hypothesis": "Canonical comprehensive validation: SOM+replay through ALL 12 validated bars (T0-T9+T13+T15+T17) on a single substrate instance.",
        "thresholds": {
            "T0_std_min": 0.05, "T1-T3_kl_min": 0.1,
            "T4_precision_min": 0.3, "T5_retention_min": 0.5,
            "T7_ratio_max": 0.10, "T8_must": "KL(AB,EN) < KL(AB,WN)",
            "T9_autocorr_min": 0.3, "T9_ratio_min": 2.0,
            "T13_coverage_min": 0.10, "T13_ratio_min": 2.0,
            "T15_ratio_max": 0.5, "T17_kl_max": 0.5,
        },
        "measurements": m,
        "schema_version": 1,
    }
