...
def test_density_count_proportional_to_amplitude():
    """G26 §3 row 1 (R-22b pre-data correction 2026-05-22): with
    DENSITY_K=8, DENSITY_N_MAX=8 the count formula must return
    n(0.0)=0, n(0.0625)=1, n(0.1875)=2, n(0.4)=3, n(0.7)=6, n(1.0)=8.

    These six fixtures are LOCKED for R-22b. K and N_MAX raised from
    4 to 8 to move the zero-injection boundary from |x|<0.125 to
    |x|<0.0625 so the substrate is non-degenerate on English audio
    (R-22 attempt 1: 64.55% of English samples below the K=4 boundary,
    substrate starved). Schoolbook rounding (floor(x + 0.5)) retained.
    """
    assert DENSITY_K == 8, f"DENSITY_K must be 8 (R-22b locked), got {DENSITY_K}"
    assert DENSITY_N_MAX == 8, (
        f"DENSITY_N_MAX must be 8 (R-22b locked), got {DENSITY_N_MAX}"
    )
    cases = [
        (0.0, 0),
        (0.0625, 1),
...
def test_density_preserves_total_energy_per_sample(monkeypatch):
    """G26 §3 row 2 (R-22b: K=8): per-sample energy budget unchanged
    from legacy.

    For ``sample_value=0.6`` under K=8, ``density_count`` returns 5;
    density mode injects 5 quanta each with energy ``|0.6| / 5 = 0.12``;
    sum = 0.6 = ``abs(sample_value)``. Verified across all n >= 1 in
    ...
    assert n == 5, (
        f"sample_value=0.6 should inject 5 quanta in density mode (K=8), got {n}"
    )
    energies = quanta.energy[quanta.alive]
    assert energies.shape == (5,), (
        f"expected exactly 5 alive quanta, got {len(energies)}"
    )
    assert np.allclose(energies, 0.12, atol=1e-12), (
        f"each density quantum at sample_value=0.6 should carry energy=0.12, "
    ...
    for sample_value in [0.0625, 0.125, 0.2, 0.4, 0.5, 0.7, 0.8, 1.0]:
