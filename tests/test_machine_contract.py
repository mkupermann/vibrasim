...
@pytest.mark.xfail(
    strict=True,
    reason=(
        "C — pattern discrimination: empirical finding (autonomous-loop "
        "iterations 4–9, 2026-05-08). The substrate produces correlated "
        "output for the LAST trained (visual, audio) pair regardless of "
        "which visual is shown in the test phase: c11=0.34 vs c12=0.51 "
        "for non-overlapping visuals across r_bridge ∈ {3, 8} and "
        "tau_LTP ∈ {0.020, 0.025}. With r_strengthen=0 (Plan A R2 "
        "disabled), STDP alone still produces this recency bias because "
        "different patterns' bridge tubes overlap once the substrate is "
        "densely seeded. Genuine discrimination requires a sparse-coding "
        "amendment — bridges that form ONLY between specific (video atom, "
        "audio atom) pairs, with hard suppression of nearby bridges that "
        "weren't part of the trained pair. This is a Plan B / STDP "
        "redesign (winner-take-all dynamics, lateral inhibition between "
        "bridge molecules), not a config tweak. Out of autonomous-loop "
        "budget; recorded as the next concrete substrate research step."
    ),
)
